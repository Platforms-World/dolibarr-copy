...
if (!function_exists('kafoerpcontrolSetConst')) {
    function kafoerpcontrolSetConst($db, $name, $value, $type = 'chaine', $visible = 0, $note = '', $entity = 1)
    {
        if (function_exists('dolibarr_set_const')) {
            return dolibarr_set_const($db, $name, $value, $type, $visible, $note, $entity);
        }

        $sql = "DELETE FROM ".MAIN_DB_PREFIX."const WHERE name='".$db->escape($name)."' AND entity=".(int) $entity;
        if (!$db->query($sql)) {
            return -1;
        }

        $sql = "INSERT INTO ".MAIN_DB_PREFIX."const(entity,name,value,type,visible,note) VALUES (".(int) $entity.", '".$db->escape($name)."', '".$db->escape((string) $value)."', '".$db->escape($type)."', ".(int) $visible.", '".$db->escape($note)."')";
...
if ($action === 'savefixed') {
    ...
    kafoerpcontrolSetConst($db, 'SAAS_API_FIXED_TOKEN', $fixedToken, 'chaine', 0, '', $conf->entity);
