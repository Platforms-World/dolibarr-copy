...
$sql = "INSERT INTO ".MAIN_DB_PREFIX."poscore_cart (entity, fk_user, fk_product, qty, price_ht, remise_percent) VALUES (".((int) $conf->entity).", ".((int) $user->id).", ".((int) $productId).", ".price2num($qty, 'MU').", ".price2num($product->price, 'MU').", 0) ON DUPLICATE KEY UPDATE qty = qty + VALUES(qty), price_ht = VALUES(price_ht), remise_percent = VALUES(remise_percent)";
if (!$db->query($sql)) {
