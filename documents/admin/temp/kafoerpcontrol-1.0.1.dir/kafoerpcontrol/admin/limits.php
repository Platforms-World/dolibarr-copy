    ...
    if ($code !== '') {
        ...
        if ($db->query($sql)) {
            saascoreAuditLogAction(
                ...
                $code,
                null,
                json_encode(array('label' => $label, 'default_value' => (int) $default_value, 'module_code' => $module_code)),
                'Limit catalog item added',
                array('context' => 'admin/limits.php')
            );
        }
    }
}
...
print dol_get_fiche_head(saascoreAdminPrepareHead(), 'limits', 'kafo-ERP-Control', -1, 'generic');
