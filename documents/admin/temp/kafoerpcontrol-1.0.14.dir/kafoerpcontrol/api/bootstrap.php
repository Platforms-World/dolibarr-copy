...
function kafoApiRequireAuth($needWrite = false)
{
    ...
    if ($needWrite) {
        $userId = !empty($GLOBALS['user']->id) ? (int) $GLOBALS['user']->id : 0;
        $hasNativeRight = is_object($GLOBALS['user']) && ($GLOBALS['user']->admin || $GLOBALS['user']->hasRight('kafoerpcontrol', 'rolemanage'));
        if (!$hasNativeRight && !$access->checkUserPermission($userId, 'kafoerp.api.write')) {
            kafoApiJson(array('success' => false, 'error' => 'Forbidden'), 403);
        }
    }
