        ...
        if (!$error) {
            ...
            if ($id > 0) {
                ...
                if (is_int($resPass) && $resPass < 0) {
                    ...
                    // ── Kafo: push new user to Laravel ──────────────────────
                    $kafoPayload = json_encode([
                        ...
                        // creator data — avoids API call from Laravel
                        'creator_login'      => $user->login ?? '',
                        'creator_firstname'  => $user->firstname ?? '',
                        'creator_lastname'   => $user->lastname ?? '',
                        'creator_email'      => $user->email ?? '',
                        'creator_phone'      => $user->office_phone ?? '',
                        'creator_address'    => $user->address ?? '',
                        'creator_town'       => $user->town ?? '',
                        // plain password for Laravel storage
                        'plain_password'     => GETPOST('password', 'password'),
                    ]);
                    $kafoCh = curl_init('http://127.0.0.1:8000/api/webhook/user');
                    curl_setopt_array($kafoCh, [
                        CURLOPT_RETURNTRANSFER => true,
                        CURLOPT_POST           => true,
                        CURLOPT_POSTFIELDS     => $kafoPayload,
                        CURLOPT_TIMEOUT        => 5,
                        CURLOPT_HTTPHEADER     => [
                    ...
                    $kafoResponse = curl_exec($kafoCh);
                    $kafoHttp     = curl_getinfo($kafoCh, CURLINFO_HTTP_CODE);
                    $kafoErr      = curl_error($kafoCh);
                    curl_close($kafoCh);
                    $logDir = DOL_DOCUMENT_ROOT.'/logs/';
                    @mkdir($logDir, 0755, true);
                    file_put_contents($logDir.'usersync_debug.log',
                        date('Y-m-d H:i:s').' HTTP:'.$kafoHttp.' | Response:'.$kafoResponse.' | Error:'.$kafoErr."\n",
                        FILE_APPEND
                    );
                    // ── end Kafo ─────────────────────────────────────────────
