...
$maxSalesPerDay = $bridge->getLimitValue($conf->entity, 'max_sales_per_day', null);
if (is_numeric($maxSalesPerDay)) {
    ...
    if ($resCount && ($objCount = $db->fetch_object($resCount)) && (int) $objCount->nb >= (int) $maxSalesPerDay) {
        http_response_code(403);
        echo json_encode(array('success' => false, 'error' => 'Daily sales limit reached'));
        exit;
    }
}
$sql = "SELECT c.fk_product, c.qty, c.price_ht, c.remise_percent, p.label, p.ref, p.tva_tx FROM ".MAIN_DB_PREFIX."poscore_cart as c INNER JOIN ".MAIN_DB_PREFIX."product as p ON p.rowid = c.fk_product AND p.entity = c.entity WHERE c.entity = ".((int) $conf->entity)." AND c.fk_user = ".((int) $user->id)." ORDER BY c.rowid ASC";
$resql = $db->query($sql);
if (!$resql) {
...
try {
    ...
    if ($invoiceId <= 0) {
        throw new Exception($invoice->error ?: implode(', ', $invoice->errors));
    }
