...
/**
 *	Class to manage PDF invoice template zatca_plus
 */
class pdf_zatca_plus extends ModelePDFFactures
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "zatca_plus";
		$this->description = $langs->trans('تنسق الفاتورة بما يتناسب مع مطلوبات هيئة الزكاة والدخل بالمملكة العربية السعودية');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				// zatca_plus QRCode
				$tag = 1;
				$value = $this->emetteur->name;
				$length = strlen($value);
				$qr_info = sprintf("%002X%002X%s",$tag, $length, bin2hex($value));

				$tag = 2;
				$value = mb_convert_encoding($this->emetteur->tva_intra ,'UTF-8');
				$length = strlen($value);
				$qr_info .= sprintf("%002X%002X%s",$tag, $length, bin2hex($value));
				...
				$value = mb_convert_encoding($timestamp,'UTF-8');
				$length = strlen($value);
				$qr_info .= sprintf("%002X%002X%s",$tag, $length, bin2hex($value));

				$tag = 4;
				$value = mb_convert_encoding(strval($object->total_ttc) ,'UTF-8');
				$length = strlen($value);
				$qr_info .= sprintf("%002X%002X%s",$tag, $length, bin2hex($value));

				$tag = 5;
				$value = mb_convert_encoding(strval($object->total_tva) ,'UTF-8');
				$length = strlen($value);
				$qr_info .= sprintf("%002X%002X%s",$tag, $length, bin2hex($value));
				$qr_info = base64_encode(pack("H*",$qr_info));
				$qr_code_module = new modTcpdfbarcode();
				dol_mkdir($conf->barcode->dir_temp);
				$qr_code_module->writeBarCode($qr_info, "QRCODE",'Y');
				$qr_image_file = $conf->barcode->dir_temp.'/barcode_'.$qr_info.'_QRCODE.png';
				$height = pdf_getHeightForLogo($qr_image_file);
				$pdf->Image($qr_image_file, $this->marge_gauche + $height  + 83, $height+$this->marge_haute + 10 , 0, $height);
				...
				$this->prepareArrayColumnFieldLocal($object, $outputlangs, $hidedetails, $hidedesc, $hideref);
				...
				for ($i = 0; $i < $nblines; $i++) {
					$curY = $nexY;
					$pdf->SetFont('aealarabiya', '', $default_font_size - 1); // Into loop to work with multipage
					$pdf->SetTextColor(0, 0, 0);
					...
					$counter =  strval($i+1);
					$this->printStdColumnContentLocal($pdf, $curY, '#', $counter);
					...
					$pdf->SetFont('aealarabiya', '', $default_font_size - 1); // We reposition the default font
				...
				// Display total zone
				$posy = $this->drawTotalTable($pdf, $object, $deja_regle, $bottomlasttab + 5, $outputlangs);
	...
	public function drawPaymentsTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		$title = $outputlangs->transnoentities("Payments");
		if ($object->type == 2) {
			$title = $outputlangs->transnoentities("PaymentsBackAlreadyDone");
		}

		$pdf->SetFont('aealarabiya', '', $default_font_size - 3);
		$pdf->SetXY($tab3_posx, $tab3_top - 4);
		$pdf->MultiCell(60, 3, $title, 0, 'L', 0);

		$pdf->line($tab3_posx, $tab3_top, $tab3_posx + $tab3_width, $tab3_top);

		$pdf->SetFont('aealarabiya', '', $default_font_size - 4);
		$pdf->SetXY($tab3_posx, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("التاريخ"), 0, 'R', 0);
		$pdf->SetXY($tab3_posx + 21, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("Amount"), 0, 'R', 0);
		$pdf->SetXY($tab3_posx + 40, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("طريقة الدفع"), 0, 'R', 0);
		$pdf->SetXY($tab3_posx + 58, $tab3_top);
		$pdf->MultiCell(20, 3, $outputlangs->transnoentities("رقم الدفعة"), 0, 'R', 0);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$pdf->MultiCell(20, 3, dol_print_date($this->db->jdate($obj->datef), 'day', false, $outputlangs, true), 0, 'R', 0);
				$pdf->SetXY($tab3_posx + 21, $tab3_top + $y);
				$pdf->MultiCell(20, 3, price((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? $obj->multicurrency_amount_ttc : $obj->amount_ttc, 0, $outputlangs), 0, 'R', 0);
				$pdf->SetXY($tab3_posx + 40, $tab3_top + $y);
				$pdf->MultiCell(20, 3, $text, 0, 'R', 0);
				$pdf->SetXY($tab3_posx + 58, $tab3_top + $y);
				$pdf->MultiCell(20, 3, $invoice->ref, 0, 'R', 0);
		...
		if ($resql) {
			...
			while ($i < $num) {
				...
				$pdf->SetXY($tab3_posx, $tab3_top + $y);
				$pdf->MultiCell(20, 3, dol_print_date($this->db->jdate($row->date), 'day', false, $outputlangs, true), 0, 'R', 0);
				$pdf->SetXY($tab3_posx + 21, $tab3_top + $y);
				$pdf->MultiCell(20, 3, price($sign * ((!empty($conf->multicurrency->enabled) && $object->multicurrency_tx != 1) ? $row->multicurrency_amount : $row->amount), 0, $outputlangs), 0, 'R', 0);
				$pdf->SetXY($tab3_posx + 40, $tab3_top + $y);
				$oper = $outputlangs->transnoentitiesnoconv("PaymentTypeShort".$row->code);

				$pdf->MultiCell(20, 3, $oper, 0, 'R', 0);
				$pdf->SetXY($tab3_posx + 58, $tab3_top + $y);
				$pdf->MultiCell(30, 3, $row->num, 0, 'R', 0);
	...
	protected function drawInfoTable(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($this->emetteur->country_code == 'FR' && empty($mysoc->tva_assuj)) {
			$pdf->SetFont('aealarabiya', 'B', $default_font_size - 2);
		...
		if ($object->type != 2) {
			...
			&& $object->mode_reglement_code != 'VIR') {
				...
				$pdf->MultiCell(80, 5, $titre, 0, 'R');

				$pdf->SetFont('aealarabiya', '', $default_font_size - 2);
				$pdf->SetXY($posxval - 20, $posy);
				$lib_mode_reg = $outputlangs->transnoentities("PaymentType".$object->mode_reglement_code) != ('PaymentType'.$object->mode_reglement_code) ? $outputlangs->transnoentities("PaymentType".$object->mode_reglement_code) : $outputlangs->convToOutputCharset($object->mode_reglement);
				$pdf->MultiCell(80, 5, $lib_mode_reg, 0, 'R');
				...
				if (($object->mode_reglement_code == 'CB' || $object->mode_reglement_code == 'VAD') && $object->statut != Facture::STATUS_DRAFT && $useonlinepayment) {
					dol_include_once('/core/lib/payments.lib.php');
					global $langs;
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'CHQ') {
				...
				if (!empty($conf->global->FACTURE_CHQ_NUMBER)) {
					...
					if ($conf->global->FACTURE_CHQ_NUMBER > 0) {
						...
						$pdf->SetXY($this->marge_gauche, $posy);
						$pdf->SetFont('aealarabiya', 'B', $default_font_size - $diffsizetitle);
						$pdf->MultiCell(100, 3, $outputlangs->transnoentities('PaymentByChequeOrderedTo', $account->proprio), 0, 'L', 0);
						$posy = $pdf->GetY() + 1;

						if (empty($conf->global->MAIN_PDF_HIDE_CHQ_ADDRESS)) {
							$pdf->SetXY($this->marge_gauche, $posy);
							$pdf->SetFont('aealarabiya', '', $default_font_size - $diffsizetitle);
	...
	protected function drawTotalTable(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if (!empty($TPreviousIncoice)) {
			...
			$pdf->SetFillColor(255, 255, 255);
			$pdf->SetXY($col1x, $posy);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("PDFSituationTitle", $object->situation_counter).' '.$outputlangs->transnoentities("TotalHT"), 0, 'R', 1);
			...
			$pdf->SetFillColor(255, 255, 255);
			$pdf->SetXY($col1x, $posy);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("SituationTotalProgress", $avancementGlobal), 0, 'R', 1);
		...
		if ($total_line_remise > 0) {
			if (!empty($conf->global->MAIN_SHOW_AMOUNT_DISCOUNT)) {
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalDiscount").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("TotalDiscount") : ''), 0, 'R', 1);
			...
			if (!empty($conf->global->MAIN_SHOW_AMOUNT_BEFORE_DISCOUNT)) {
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalHTBeforeDiscount").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("TotalHTBeforeDiscount") : ''), 0, 'R', 1);
		...
		$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalHT").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("TotalHT") : ''), 0, 'R', 1);
			...
			if (!empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT_IFNULL) && $tvaisnull) {
				...
				foreach ($this->tva as $tvakey => $tvaval) {
					if ($tvakey != 0) {    // On affiche pas taux 0
						...
						$pdf->MultiCell($col2x - $col1x, $tab2_hl, $totalvat, 0, 'R', 1);
				...
				if (price2num($object->revenuestamp) != 0) {
					$index++;
					$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
					$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("RevenueStamp").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("RevenueStamp", $mysoc->country_code) : ''), $useborder, 'R', 1);
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("TotalTTC").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("TotalTTC") : ''), $useborder, 'R', 1);
		...
		if (($deja_regle > 0 || $creditnoteamount > 0 || $depositsamount > 0) && empty($conf->global->INVOICE_NO_PAYMENT_DETAILS)) {
			...
			$index++;
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("Paid").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("Paid") : ''), 0, 'R', 0);
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
			...
			*/
			...
			$pdf->SetFillColor(224, 224, 224);
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("RemainderToPay").(is_object($outputlangsbis) ? ' / '.$outputlangsbis->transnoentities("RemainderToPay") : ''), $useborder, 'R', 1);
			$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		$pdf->SetTextColor(0, 0, 60);
		$pdf->SetFont('aealarabiya', 'B', $default_font_size + 3);
		...
		if (empty($conf->global->PDF_DISABLE_MYCOMPANY_LOGO)) {
			if ($this->emetteur->logo) {
				...
				if (is_readable($logo)) {
					...
					$pdf->Image($logo, $this->marge_gauche + $height + 160 , $posy, 0, $height); // width=0 (auto)
				} else {
					$pdf->SetTextColor(200, 0, 0);
					$pdf->SetFont('aealarabiya', 'B', $default_font_size - 2);
		...
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("فاتورة ضريبية");
		if ($object->type == 1) {
		...
		if (!empty($conf->global->PDF_USE_ALSO_LANGUAGE_CODE) && is_object($outputlangsbis)) {
			...
			if ($object->type == 0) {
				...
				$title .= $outputlangsbis->transnoentities("Invoice Tax");
			} elseif ($object->type == 1) {
				$title .= $outputlangsbis->transnoentities("Invoice Replacement");
			} elseif ($object->type == 2) {
				$title .= $outputlangsbis->transnoentities("Invoice Avoir");
			} elseif ($object->type == 3) {
				$title .= $outputlangsbis->transnoentities("Invoice Deposit");
			} elseif ($object->type == 4) {
				$title .= $outputlangsbis->transnoentities("Invoice ProForma");
			}
		}
		$pdf->MultiCell($w, 3, $title, '', 'R');

		$pdf->SetFont('aealarabiya', 'B', $default_font_size);

		$posy += 10;
		$pdf->SetXY($posx + 133, $posy);
		$pdf->SetTextColor(0, 0, 60);
		$textref = $outputlangs->transnoentities("رقم الفاتورة")." : ".$outputlangs->convToOutputCharset($object->ref);
		...
		$pdf->MultiCell($w, 3, $title." : ".dol_print_date($object->date, "%Y-%m-%d", false, $outputlangs, true), '', 'R');

		if (!empty($conf->global->INVOICE_POINTOFTAX_DATE)) {
			$posy += 4;
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, $outputlangs->transnoentities("DatePointOfTax")." : ".dol_print_date($object->date_pointoftax, "%Y-%m-%d", false, $outputlangs), '', 'R');
		}
		...
		if ($object->type != 2) {
			...
			$pdf->MultiCell($w, 3, $title." : ".dol_print_date($object->date_lim_reglement, "%Y-%m-%d", false, $outputlangs, true), '', 'R');
		}
		...
		if ($showaddress) {
			...
			$pdf->MultiCell(66, 5, $outputlangs->transnoentities("BillFrom").":", 0, 'R');
			$pdf->SetXY($posx, $posy);
			...
			$pdf->SetFont('aealarabiya', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox - 2, 4, $outputlangs->convToOutputCharset($this->emetteur->name), 0, 'R');
			$posy = $pdf->getY();

			// Show sender information
			$pdf->SetXY($posx + 2, $posy);
			$pdf->SetFont('aealarabiya', '', $default_font_size - 1);
			$pdf->MultiCell($widthrecbox - 2, 4, $carac_emetteur, 0, 'R');
			...
			$posy += 5;
			$posy += $top_shift;
			$posx = $this->page_largeur - $this->marge_droite - $widthrecbox + 30;
			if (!empty($conf->global->MAIN_INVERT_SENDER_RECIPIENT)) {
				$posx = $this->marge_gauche;
			}

			// Show recipient frame =>  Todo
			$pdf->SetTextColor(0, 0, 0);
			$pdf->SetFont('aealarabiya', '', $default_font_size - 2);
			$pdf->SetXY($posx , $posy - 5 );
			$pdf->MultiCell($widthrecbox, 5, $outputlangs->transnoentities("BillTo").":", 0, 'R');
			//$pdf->Rect($posx + 10, $posy - 10 , $widthrecbox, $hautcadre);

			// Show recipient name
			$pdf->SetXY($posx + 2, $posy + 3);
			$pdf->SetFont('aealarabiya', 'B', $default_font_size);
			$pdf->MultiCell($widthrecbox, 2, $carac_client_name, 0, 'R');
		...
		*/
		...
		$this->cols['desc'] = array(
			...
			'title' => array(
				...
				'padding' => array(0.5, 7.0, 0.5, 0.5), // Like css 0 => top , 1 => right, 2 => bottom, 3 => left
			),
			'content' => array(
				'align' => 'R',
				'padding' => array(1, 7.0, 1, 0.5), // Like css 0 => top , 1 => right, 2 => bottom, 3 => left
			),
		);
		...
		$this->cols['photo'] = array(
			...
			'border-right' => false, // remove left line separator
		);
		...
		$this->cols['vat'] = array(
			...
			'border-right' => true, // add left line separator
		);
	 ...
	 *      @return	null
	 */
	public function prepareArrayColumnFieldLocal($object, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		global $conf;
	...
	public function pdfTabTitles(&$pdf, $tab_top, $tab_height, $outputlangs, $hidetop = 0)
	{
		...
		foreach ($this->cols as $colKey => $colDef) {
			...
			if ($reshook < 0) {
				...
				if (!empty($colDef['border-right'])) {
					// FIXME add the right border positions
					$pdf->line(195, $tab_top, 195, $tab_top + $tab_height);
					$pdf->line(90, $tab_top, 90, $tab_top + $tab_height);
					$pdf->line(71, $tab_top, 71, $tab_top + $tab_height);
					$pdf->line(52, $tab_top, 52, $tab_top + $tab_height);
					$pdf->line(37, $tab_top, 37, $tab_top + $tab_height);
				}
...
/**
	 ...
	 *  @return	null
	 */
	public function printStdColumnContentLocal($pdf, &$curY, $colKey, $columnText = '')
	{
		global $hookmanager;
		...
		if (!$reshook) {
			...
			$pdf->writeHTMLCell($colDef['width'], 2, $colDef['xStartPos'] + 11, $curY, $columnText, 0, 1, 0, true, $colDef['content']['align']);
