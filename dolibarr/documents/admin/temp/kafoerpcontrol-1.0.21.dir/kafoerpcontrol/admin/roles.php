...
if ($selectedRoleCode !== '' && isset($roles[$selectedRoleCode])) {
    ...
    print '<form method="POST" action="' . kafoRoleEscape($_SERVER['PHP_SELF']) . '" id="kafo-role-permissions-form" style="margin-bottom:16px;">';
    print '<input type="hidden" name="token" value="' . newToken() . '">';
    ...
    print '<input type="button" class="button" value="Select All" onclick="kafoToggleRolePermissions(true)"> ';
    print '<input type="button" class="button" value="Unselect All" onclick="kafoToggleRolePermissions(false)"> ';
    print '<span class="opacitymedium">Quick selection for all visible permissions in this group.</span>';
    print '</td></tr>';
    ...
    foreach ($permissions as $code => $perm) {
        print '<tr class="oddeven">';
        print '<td style="width:60px;"><input class="kafo-role-permission-checkbox" type="checkbox" name="role_permissions[' . kafoRoleEscape($code) . ']" value="1"' . (!empty($assignedPermissions[$code]) ? ' checked' : '') . '></td>';
    ...
    print '</table></form>';
    print '<script>';
    print 'function kafoToggleRolePermissions(flag){var nodes=document.querySelectorAll("#kafo-role-permissions-form .kafo-role-permission-checkbox");for(var i=0;i<nodes.length;i++){if(!nodes[i].disabled){nodes[i].checked=!!flag;}}}';
    print '</script>';
