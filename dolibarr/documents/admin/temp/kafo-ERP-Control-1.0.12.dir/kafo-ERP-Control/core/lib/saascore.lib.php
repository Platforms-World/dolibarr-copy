...
function saascoreAdminPrepareHead()
{
    ...
    $h = 0;
    $head = array();
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/index.php', 1), $langs->trans('General'), 'general');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/modules.php', 1), $langs->trans('ModulesCatalog'), 'modules');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/features.php', 1), $langs->trans('FeaturesCatalog'), 'features');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/limits.php', 1), $langs->trans('LimitsCatalog'), 'limits');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/permissions.php', 1), $langs->trans('PermissionsCatalog'), 'permissions');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/bundles.php', 1), $langs->trans('BundlesCatalog'), 'bundles');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/roles.php', 1), $langs->trans('RolesCatalog'), 'roles');
    $head[$h++] = array(dol_buildpath('/kafo-ERP-Control/admin/tenant.php', 1), $langs->trans('TenantConfiguration'), 'tenant');
    return $head;
}
...
function saascoreRegisterTakeposCatalog($db)
{
    ...
    $registryFile = dol_buildpath('/kafo-ERP-Control/class/SaasRegistryService.php', 0);
