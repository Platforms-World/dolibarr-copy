...
if ($mainInc === null) {
    ...
    print 'Unable to locate Dolibarr main.inc.php from ' . htmlspecialchars(__DIR__, ENT_QUOTES, 'UTF-8');
    exit;
}
...
$langs->loadLangs(array('admin', 'users', 'other', 'saascore@kafo-ERP-Control'));

$canManageRights = false;
if (!empty($user->admin)) {
    $canManageRights = true;
} elseif (
    $user->hasRight('saascore', 'nativerightsmanage')
...
function kafoPcGetUserDirectRights($db, $userId, array $availableIds)
{
    ...
    $sql = 'SELECT ur.fk_id';
    $sql .= ' FROM ' . MAIN_DB_PREFIX . 'userrights as ur';
...
if ($action === 'save' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($postedUserId <= 0 || !isset($users[$postedUserId])) {
        ...
        foreach ($toAdd as $rightId) {
            $sql = 'INSERT INTO ' . MAIN_DB_PREFIX . 'userrights (fk_user, fk_id)';
            $sql .= ' SELECT ' . ((int) $postedUserId) . ', ' . ((int) $rightId);
            $sql .= ' FROM DUAL WHERE NOT EXISTS (';
            $sql .= 'SELECT 1 FROM ' . MAIN_DB_PREFIX . 'userrights';
        ...
        if ($ok && !empty($toDelete)) {
            $sql = 'DELETE FROM ' . MAIN_DB_PREFIX . 'userrights';
...
print dol_get_fiche_head(saascoreAdminPrepareHead(), 'permissionscontrol', $langs->trans('kafo-ERP-Control'), -1, 'generic');
...
print '<option value="">' . kafoPcEscape('Core') . '</option>';
...
print '<option value="all">' . kafoPcEscape('Core') . '</option>';
...
foreach ($rights as $right) {
    ...
    print '<td><input type="checkbox" class="right-checkbox" name="rights[' . $rid . ']" value="1" ' . ($isDirect ? 'checked' : '') . '></td>';
    print '</tr>';
}
