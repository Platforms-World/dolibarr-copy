...
function saas_ensure_tables($db)
{
    ...
    // Audit schema is best-effort and must never block save flow.
...
function saas_audit($db, $entityId, $actorUserId, $actionCode, $targetType, $targetCode, $oldValue, $newValue)
{
    ...
    saascoreAuditLogAction(
        ...
        array('entity_id' => (int) $entityId)
    );

    // Logging must never block save flow.
    return true;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $action === 'save') {
    ...
    if ($ok) {
        ...
        foreach ($newModules as $moduleCode => $enabled) {
            $oldEnabled = isset($oldModules[$moduleCode]) ? (int) $oldModules[$moduleCode] : 0;
            if ((int) $enabled !== $oldEnabled) {
                saascoreAuditLogAction($db, $actorUserId, 0, 'module_status_update', 'module', (string) $moduleCode, $oldEnabled, (int) $enabled, 'Tenant module status changed', array('entity_id' => $entityId));
            }
        }
        foreach ($oldModules as $moduleCode => $enabled) {
            if (!isset($newModules[$moduleCode])) {
                saascoreAuditLogAction($db, $actorUserId, 0, 'module_status_update', 'module', (string) $moduleCode, (int) $enabled, 0, 'Tenant module status changed', array('entity_id' => $entityId));
            }
        }

        foreach ($newFeatures as $featureCode => $enabled) {
            $oldEnabled = isset($oldFeatures[$featureCode]) ? (int) $oldFeatures[$featureCode] : 0;
            if ((int) $enabled !== $oldEnabled) {
                saascoreAuditLogAction($db, $actorUserId, 0, 'feature_flag_update', 'feature', (string) $featureCode, $oldEnabled, (int) $enabled, 'Tenant feature flag changed', array('entity_id' => $entityId));
            }
        }
        foreach ($oldFeatures as $featureCode => $enabled) {
            if (!isset($newFeatures[$featureCode])) {
                saascoreAuditLogAction($db, $actorUserId, 0, 'feature_flag_update', 'feature', (string) $featureCode, (int) $enabled, 0, 'Tenant feature flag changed', array('entity_id' => $entityId));
            }
        }

        foreach ($newLimits as $limitCode => $value) {
            $oldValue = isset($oldLimits[$limitCode]) ? (int) $oldLimits[$limitCode] : null;
            if ((string) $oldValue !== (string) ((int) $value)) {
                saascoreAuditLogAction($db, $actorUserId, 0, 'limit_update', 'limit', (string) $limitCode, $oldValue, (int) $value, 'Tenant limit changed', array('entity_id' => $entityId));
            }
        }

        foreach ($newRoles as $roleCode => $assigned) {
            if (empty($oldRoles[$roleCode])) {
                saascoreAuditLogAction($db, $actorUserId, (int) $selectedUserId, 'role_add', 'user', (string) $roleCode, null, 'assigned', 'Role assigned from tenant configuration', array('entity_id' => $entityId));
            }
        }
        foreach ($oldRoles as $roleCode => $assigned) {
            if (empty($newRoles[$roleCode])) {
                saascoreAuditLogAction($db, $actorUserId, (int) $selectedUserId, 'role_remove', 'user', (string) $roleCode, 'assigned', null, 'Role removed from tenant configuration', array('entity_id' => $entityId));
            }
        }

        foreach ($newUserPerms as $permCode => $allowed) {
            if (empty($oldUserPerms[$permCode])) {
                saascoreAuditLogAction($db, $actorUserId, (int) $selectedUserId, 'permission_add', 'user', (string) $permCode, null, '1', 'Direct user permission added from tenant configuration', array('entity_id' => $entityId));
            }
        }
        foreach ($oldUserPerms as $permCode => $allowed) {
            if (empty($newUserPerms[$permCode])) {
                saascoreAuditLogAction($db, $actorUserId, (int) $selectedUserId, 'permission_remove', 'user', (string) $permCode, '1', null, 'Direct user permission removed from tenant configuration', array('entity_id' => $entityId));
            }
        }
