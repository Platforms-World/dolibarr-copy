...
function get_left_menu_home($mainmenu, &$newmenu, $usemenuhider = 1, $leftmenu = 'none', $type_user = 0)
{
    ...
    if ($mainmenu == 'home') {
        ...
        // Setup
        $newmenu->add("/admin/index.php?mainmenu=home&amp;leftmenu=setup", $langs->trans("Setup"), 0, ($user->admin && empty($conf->global->KAFO_LOCK_MODULES)), '', $mainmenu, 'setup', 0, '', '', '', '<i class="fa fa-tools fa-fw paddingright pictofixedwidth"></i>');
        ...
        if ($usemenuhider || empty($leftmenu) || $leftmenu == "setup") {
            ...
            if (empty($conf->global->KAFO_LOCK_MODULES)) {
        ...
        // System tools
        if (isModEnabled('kafoerpcontrol')) {
            $newmenu->add("/custom/kafoerpcontrol/admin/index.php?mainmenu=home&leftmenu=saascore", 'kafo-ERP-Control', 0,
                ($user->admin || $user->hasRight('kafoerpcontrol', 'read')),
                '', $mainmenu, 'saascore', 0, '', '', '', '');
            if ($usemenuhider || empty($leftmenu) || $leftmenu == 'saascore') {
                $newmenu->add("/custom/kafoerpcontrol/admin/tenant.php", 'Tenant Configuration', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'tenantmanage'));
                $newmenu->add("/custom/kafoerpcontrol/admin/modules.php", 'Modules Catalog', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'write'));
                $newmenu->add("/custom/kafoerpcontrol/admin/features.php", 'Features Catalog', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'write'));
                $newmenu->add("/custom/kafoerpcontrol/admin/limits.php", 'Limits Catalog', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'write'));
                $newmenu->add("/custom/kafoerpcontrol/admin/permissions.php", 'Permissions Catalog', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'rolemanage'));
                $newmenu->add("/custom/kafoerpcontrol/admin/permissions_control.php", 'Permissions Control', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'nativerightsmanage'));
                $newmenu->add("/custom/kafoerpcontrol/admin/bundles.php", 'Bundles Catalog', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'write'));
                $newmenu->add("/custom/kafoerpcontrol/admin/roles.php", 'Roles Catalog', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'rolemanage'));
                $newmenu->add("/custom/kafoerpcontrol/admin/api.php", 'API', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'apimanage'));
                $newmenu->add("/custom/kafoerpcontrol/admin/auditlog.php", 'Audit Log', 1, $user->admin || $user->hasRight('kafoerpcontrol', 'read'));
            }
        }
