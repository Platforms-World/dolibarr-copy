    ...
    if ($code !== '') {
        ...
        if ($db->query($sql)) {
            saascoreAuditLogAction(
                ...
                $actorUserId,
                0,
                'permission_catalog_add',
                'permission',
                $code,
                null,
                json_encode(array('label' => $label, 'module_code' => $module_code)),
                'Permission catalog item added',
                array('context' => 'admin/permissions.php')
            );
        }
    }
}
...
print dol_get_fiche_head(saascoreAdminPrepareHead(), 'permissions', 'kafo-ERP-Control', -1, 'generic');
