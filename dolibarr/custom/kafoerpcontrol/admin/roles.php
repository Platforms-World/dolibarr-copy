...
require_once dol_buildpath('/kafoerpcontrol/class/SaasNativeRightsSyncService.php', 0);
...
$syncService = new SaasNativeRightsSyncService($db);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create_role') {
        ...
        if ($code === '' || $label === '') {
            ...
            if ($service->createRole($entityId, $code, $label, $description, $isSystem)) {
                saascoreAuditLogAction(
                    ...
                    $code,
                    null,
                    json_encode(array('label' => $label, 'description' => $description, 'is_system' => (int) $isSystem)),
                    'Role created',
            ...
            if ($label === '') {
                ...
                if ($db->query($sql)) {
                    saascoreAuditLogAction(
                        ...
                        'role',
                        $roleCode,
                        json_encode(array('label' => $existingRole->label, 'description' => $existingRole->description, 'is_system' => (int) $existingRole->is_system)),
                        json_encode(array('label' => $label, 'description' => $description, 'is_system' => (int) $isSystem)),
                        'Role updated',
                        array('entity_id' => $entityId)
                    );
        ...
        if (!isset($roles[$roleCode])) {
            ...
            $currentPermissions = kafoRoleGetAssignedPermissions($db, $entityId, $roleCode);
            $db->begin();
            $ok = true;
            $sqls = array(
            ...
            if ($ok) {
                ...
                foreach (array_keys($currentUsers) as $targetUserId) {
                    saascoreAuditLogAction(
                        $db,
                        $actorUserId,
                        (int) $targetUserId,
                        ...
                        'assigned',
                        'removed',
                        'Role removed from user during role delete',
                        array('entity_id' => $entityId)
                    );
                }
                foreach ($currentPermissions as $permCode => $allowed) {
                    saascoreAuditLogAction(
                        $db,
                        $actorUserId,
                        0,
                        ...
                        (int) $allowed,
                        null,
                        'Role permission removed during role delete',
                        array('entity_id' => $entityId, 'role_code' => $roleCode, 'permission_code' => $permCode)
                    );
                }
                saascoreAuditLogAction(
                    $db,
                    ...
                    'role',
                    $roleCode,
                    json_encode(array('users_count' => count($currentUsers), 'permissions_count' => count($currentPermissions))),
                    null,
                    'Role deleted',
                    array('entity_id' => $entityId)
                );
        ...
        if (!isset($roles[$roleCode])) {
            ...
            if ($ok) {
                ...
                foreach ($toAdd as $permissionCode) {
                    saascoreAuditLogAction(
                        $db,
                        $actorUserId,
                        0,
                        ...
                        array('entity_id' => $entityId, 'role_code' => $roleCode, 'permission_code' => $permissionCode)
                    );
                }
                foreach ($toDelete as $permissionCode) {
                    saascoreAuditLogAction(
                        $db,
                        $actorUserId,
                        0,
            ...
            if ($ok) {
                ...
                foreach ($toAdd as $targetUserId) {
                    saascoreAuditLogAction(
                        ...
                        array('entity_id' => $entityId, 'role_code' => $roleCode)
                    );
                }
                foreach ($toDelete as $targetUserId) {
                    saascoreAuditLogAction(
                        $db,
                        $actorUserId,
                        (int) $targetUserId,
...
if ($selectedRoleCode !== '' && isset($roles[$selectedRoleCode])) {
    ...
    print '<input type="button" class="button" value="Select All Permissions" onclick="kafoToggleCheckboxes(\'role-permission-checkbox\', true)"> ';
    print '<input type="button" class="button" value="Unselect All Permissions" onclick="kafoToggleCheckboxes(\'role-permission-checkbox\', false)">';
    print '</div>';
    print '<table class="noborder centpercent">';
    ...
    foreach ($permissions as $code => $perm) {
        print '<tr class="oddeven">';
        print '<td style="width:60px;"><input type="checkbox" class="role-permission-checkbox" name="role_permissions[' . kafoRoleEscape($code) . ']" value="1"' . (!empty($assignedPermissions[$code]) ? ' checked' : '') . '></td>';
    ...
    print '<input type="hidden" name="role_code" value="' . kafoRoleEscape($selectedRoleCode) . '">';
    print '<div style="margin:8px 0;">';
    print '<input type="button" class="button" value="Select All Users" onclick="kafoToggleCheckboxes(\'role-user-checkbox\', true)"> ';
    print '<input type="button" class="button" value="Unselect All Users" onclick="kafoToggleCheckboxes(\'role-user-checkbox\', false)">';
    print '</div>';
    print '<table class="noborder centpercent">';
    ...
    foreach ($users as $userId => $u) {
        ...
        print '<tr class="oddeven">';
        print '<td style="width:60px;"><input type="checkbox" class="role-user-checkbox" name="role_users[' . ((int) $userId) . ']" value="1"' . (!empty($assignedUsers[$userId]) ? ' checked' : '') . '></td>';
...
print '</div>';
print '<script>
function kafoToggleCheckboxes(className, checked) {
