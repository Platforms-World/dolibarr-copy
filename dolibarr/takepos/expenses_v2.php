...
$pageUrl = DOL_URL_ROOT . '/takepos/expenses_v2.php';
TakeposAccess::requireFrontendAccess(
    ...
    array('page' => 'expenses_v2.php')
);

if (!TakeposExpenseService::canRead($db, $user)) {
    TakeposAccess::denyAccess($db, $user, $langs->trans('TakeposExpenseReadPermissionRequired'), array('page' => 'expenses_v2.php', 'permission' => 'takepos.expense.read'));
}

TakeposAudit::logEvent($db, $user, 'expense_screen_opened', TakeposAudit::SEVERITY_INFO, array('page' => 'expenses_v2.php'), 'POS expense screen opened');
...
<?php
$v2PageTitle = $langs->trans('TakeposExpenseTitle');
$v2PageIcon  = 'fa-money-bill-wave';
...
<div class="kfv2-page-body" style="max-width:1460px;margin:0 auto;padding:22px 26px 48px">
    <h2 style="display:none"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseTitle')); ?></h2>
    ...
    <section class="kfv2-card-block">
        <div class="kfv2-card-block-head"><h3 style="margin:0;font-size:14.5px"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseDetails')); ?></h3></div><div class="kfv2-card-block-body">
        <form method="post" action="<?php echo dol_escape_htmltag($pageUrl . ($expenseId > 0 ? '?id=' . ((int) $expenseId) : '')); ?>">
            ...
            <input type="kfv2-hidden" name="expense_id" value="<?php echo (int) $formValues['expense_id']; ?>">
            ...
            <div class="kfv2-actions">
                <?php if ($canCreateExpense && !empty($categories)) { ?>
                    <button type="submit" name="action" value="save" class="kfv2-btn kfv2-btn-outline"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseSaveDraft')); ?></button>
                <?php } ?>
                <?php if ($canPostExpense && !empty($categories)) { ?>
                    <button type="submit" name="action" value="save_post" class="kfv2-btn kfv2-btn-primary"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseSaveAndPost')); ?></button>
                <?php } ?>
                <button type="button" class="kfv2-btn kfv2-btn-outline" onclick="takeposCloseExpenseScreen();"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonCancelBack')); ?></button>
            </div>
        </form>
    </section>

    <section class="kfv2-card-block">
        <div class="kfv2-card-block-head"><h3 style="margin:0;font-size:14.5px"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseAccountingSummary')); ?></h3></div><div class="kfv2-card-block-body">
        <div class="kfv2-kpi" style="max-width:none;">
    ...
    <section class="kfv2-card-block">
        <div class="kfv2-card-block-head"><h3 style="margin:0;font-size:14.5px"><?php echo dol_escape_htmltag($langs->trans('TakeposExpensePostingStatus')); ?></h3></div><div class="kfv2-card-block-body">
    ...
    <section class="kfv2-card-block">
        <div class="kfv2-card-block-head"><h3 style="margin:0;font-size:14.5px"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseRecentExpenses')); ?></h3></div><div class="kfv2-card-block-body">
        <div class="kfv2-table-wrap">
            <table class="kfv2-table">
                <thead>
                <tr>
                ...
                <?php } else { ?>
                    <?php foreach ($recentExpenses as $row) { ?>
                        <tr>
                            ...
                            <td><a class="kfv2-btn kfv2-btn-outline" href="<?php echo dol_escape_htmltag($pageUrl . '?id=' . ((int) $row->rowid)); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonView')); ?></a></td>
                        </tr>
                    <?php } ?>
                <?php } ?>
                </tbody>
...
(function () {
    ...
    window.takeposCloseExpenseScreen = function () {
        ...
        window.location.href = '<?php echo dol_escape_js(DOL_URL_ROOT . '/takepos/pos_v2.php'); ?>';
    };
