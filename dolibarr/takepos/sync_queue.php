...
TakeposAccess::requireFrontendAccess($db, $user, 'takepos.sync_queue', 'takepos.sync.manage', $terminal, $langs->trans('TakeposWorkspaceSyncAccessDenied'), array('page' => 'sync_queue.php'));
...
<div class="takepos-workspace-reports-page">
    <h2 class="takepos-workspace-title"><?php echo dol_escape_htmltag($langs->trans('TakeposSyncQueueTitle')); ?></h2>
    ...
    <div id="takepos-sync-feedback" class="takepos-workspace-feedback hidden" role="status" aria-live="polite"></div>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposSyncQueueStatus')); ?></h3>
        <div class="takepos-workspace-summary-cards" id="sync_cards"></div>
        <div class="takepos-workspace-filter-actions">
            <button type="button" id="btn_refresh" class="button"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonRefresh')); ?></button>
            <button type="button" id="btn_process_pending" class="button button-save"><?php echo dol_escape_htmltag($langs->trans('TakeposSyncQueueProcessPending')); ?></button>
        </div>
    </section>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposSyncQueueFilters')); ?></h3>
    ...
    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposSyncQueueQueueEntries')); ?></h3>
        <div class="takepos-workspace-table-wrap"><table id="table_queue" class="takepos-workspace-table"></table></div>
    </section>
</div>
...
(function () {
    ...
    function renderCards(summary) {
        ...
        keys.forEach(function (k) {
            html += '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n[k] || k) + '</div><div class="takepos-workspace-card-value">' + safe(summary && summary[k] ? summary[k] : 0) + '</div></div>';
        });
        node.innerHTML = html;
    }
