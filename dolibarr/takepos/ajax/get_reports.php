...
function takeposBuildWhereClause($db, $filters, $withLine = false)
{
    ...
    $where[] = "f.module_source COLLATE utf8mb4_general_ci = 'takepos'";
...
try {
    ...
    if ($action === 'filters') {
        $whereTakepos = 'f.entity = ' . ((int) $conf->entity) . " AND f.module_source COLLATE utf8mb4_general_ci = 'takepos'";
    ...
    FROM " . MAIN_DB_PREFIX . "facture f
    INNER JOIN " . MAIN_DB_PREFIX . "facturedet fd ON fd.fk_facture = f.rowid
    LEFT JOIN " . MAIN_DB_PREFIX . "takepos_terminal tt ON tt.entity = f.entity AND tt.terminal_code COLLATE utf8mb4_general_ci = f.pos_source COLLATE utf8mb4_general_ci
    WHERE " . $whereLine . "
    ...
    // Receivables: all unpaid customer invoices (paye=0), any status including Draft.
    $receivableWhere = array(
        'f.entity = ' . ((int) $conf->entity),
        'f.paye = 0',
        'f.type NOT IN (2, 3)',   // exclude credit notes and deposits
    );
    if (!empty($filters['customer_id'])) {
    ...
    $sqlReceivables = "SELECT f.ref, COALESCE(s.nom, '') AS customer_name, COALESCE(f.datef, f.datec) AS invoice_date, f.date_lim_reglement AS due_date, f.total_ttc, COALESCE(pay.paid_amount, 0) AS paid_amount, (f.total_ttc - COALESCE(pay.paid_amount, 0)) AS remaining_amount, "
        ...
        . " ORDER BY f.date_lim_reglement ASC, f.rowid DESC"
        . $db->plimit(500, 0);
    ...
    if (takeposReportsTableExists($db, $supplierInvoiceTable)) {
        ...
        // Payables: all unpaid supplier invoices (paye=0), including Draft.
        $payableWhere = array(
        ...
        $sqlPayables = "SELECT ff.ref, COALESCE(s.nom, '') AS supplier_name, COALESCE(ff.datef, ff.datec) AS invoice_date, ff.date_lim_reglement AS due_date, ff.total_ttc, " . $supplierPaidExpr . " AS paid_amount, (ff.total_ttc - " . $supplierPaidExpr . ") AS remaining_amount, "
            ...
            . " ORDER BY ff.date_lim_reglement ASC, ff.rowid DESC"
            . $db->plimit(500, 0);
    ...
    $nearExpiry = array();
    // Read expiry from kafo_expiry_date extrafield (primary) AND llx_product_batch (secondary)
    $expiryDateExpr90 = "DATE_ADD(CURDATE(), INTERVAL 90 DAY)";
    $nearExpiryProductFilter = '';
    if (!empty($filters['product_id'])) {
        $nearExpiryProductFilter = ' AND p.rowid = ' . ((int) $filters['product_id']);
    }

    // Source 1: kafo_expiry_date extrafield (products without lot/serial tracking)
    $extrafieldsTable = MAIN_DB_PREFIX . 'product_extrafields';
    if (takeposReportsTableExists($db, $extrafieldsTable)
        && takeposReportsColumnExists($db, $extrafieldsTable, 'kafo_expiry_date')) {
        $sqlExtraExpiry = "SELECT"
            . " COALESCE(p.ref, '') AS product_ref,"
            . " COALESCE(p.label, '') AS product_label,"
            . " '' AS batch,"
            . " '' AS warehouse_name,"
            . " COALESCE(p.stock, 0) AS qty,"
            . " DATE(pe.kafo_expiry_date) AS expiry_date,"
            . " CASE WHEN pe.kafo_expiry_date < CURDATE() THEN 'expired'"
            . "      WHEN pe.kafo_expiry_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN 'near_expiry'"
            . "      ELSE 'watch' END AS expiry_status"
            . " FROM " . $extrafieldsTable . " pe"
            . " INNER JOIN " . MAIN_DB_PREFIX . "product p ON p.rowid = pe.fk_object"
            . " WHERE p.entity IN (" . getEntity('product') . ")"
            . "   AND pe.kafo_expiry_date IS NOT NULL"
            . "   AND pe.kafo_expiry_date != '0000-00-00'"
            . "   AND pe.kafo_expiry_date <= " . $expiryDateExpr90
            . $nearExpiryProductFilter
            . " ORDER BY pe.kafo_expiry_date ASC, p.label ASC"
            . $db->plimit(500, 0);
        $nearExpiry = takeposFetchRows($db, $sqlExtraExpiry);
    }

    // Source 2: llx_product_batch (products with lot/serial tracking)
    if (takeposReportsTableExists($db, MAIN_DB_PREFIX . 'product_batch')
        && takeposReportsTableExists($db, MAIN_DB_PREFIX . 'product_stock')) {
        $expiryExpr = 'COALESCE(pb.sellby, pb.eatby)';
        $sqlBatchExpiry = "SELECT"
            . " COALESCE(p.ref, '') AS product_ref,"
            . " COALESCE(p.label, '') AS product_label,"
            . " pb.batch,"
            . " COALESCE(NULLIF(e.ref, ''), NULLIF(e.label, ''), NULLIF(e.lieu, ''), '') AS warehouse_name,"
            . " COALESCE(pb.qty, 0) AS qty,"
            . " DATE(" . $expiryExpr . ") AS expiry_date,"
            . " CASE WHEN " . $expiryExpr . " < CURDATE() THEN 'expired'"
            . "      WHEN " . $expiryExpr . " <= DATE_ADD(CURDATE(), INTERVAL 30 DAY) THEN 'near_expiry'"
            . "      ELSE 'watch' END AS expiry_status"
            ...
            . " WHERE " . $expiryExpr . " IS NOT NULL"
            . "   AND " . $expiryExpr . " <= " . $expiryDateExpr90
            . $nearExpiryProductFilter
            . " ORDER BY " . $expiryExpr . " ASC, p.label ASC"
            . $db->plimit(500, 0);
        $batchExpiry = takeposFetchRows($db, $sqlBatchExpiry);
        // Merge: avoid duplicates (same product may appear in both sources)
        $existingRefs = array_column($nearExpiry, 'product_ref');
        foreach ($batchExpiry as $row) {
            if (!in_array($row['product_ref'], $existingRefs)) {
                $nearExpiry[] = $row;
                $existingRefs[] = $row['product_ref'];
            }
        }
        // Re-sort by expiry_date
        usort($nearExpiry, function($a, $b) {
