...
// FIX (stock-branch-v1): Branch users now go through the same stock check as
// master terminal users. The old bypass caused branches to oversell without
// any inventory impact. Each branch must have a warehouse assigned
// (fk_warehouse on llx_takepos_branch / CASHDESK_ID_WAREHOUSE{N} on the
// branch terminal). If no warehouse is configured for this terminal, the
// check falls back to sum-of-all-warehouses, matching master terminal behaviour.
...
$stockAvailable = 0;
// FIX: Always check total stock across ALL warehouses.
// The merchant can have stock in any warehouse — what matters is the total.
// Stock deduction on sale uses fk_default_warehouse per product (in invoice.php).
