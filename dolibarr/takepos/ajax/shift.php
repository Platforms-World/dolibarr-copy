...
try {
    ...
    if ($action === 'list') {
        ...
        // ── Branch user shift isolation ──────────────────────────────────────
        // Branch users must only see shifts for their own terminal(s).
        // Without this, they see every shift in the system.
        if (empty($user->admin)) {
            ...
            if ($branch) {
                // Get all terminal rowids belonging to this branch
                $branchTerminals = TakeposBranchService::getTerminalsForBranchId($db, (int) $branch->rowid);
                $branchTerminalIds = array_keys($branchTerminals);

                if (empty($branchTerminalIds)) {
                    // Branch has no terminals — return empty list, not all shifts
                    takeposShiftJson(array('success' => true, 'rows' => array()));
                    exit;
                }

                // If a specific terminal_id was requested, verify it belongs to this branch
                if (!empty($filters['terminal_id']) && !in_array((int) $filters['terminal_id'], $branchTerminalIds, true)) {
                    takeposShiftJson(array('success' => true, 'rows' => array()));
                    exit;
                }

                // If no terminal filter set, restrict to branch terminals only
                if (empty($filters['terminal_id'])) {
                    // Use first branch terminal — branch users have exactly one
                    $filters['terminal_id'] = (int) $branchTerminalIds[0];
                }
            }
        }
    ...
    if ($action === 'detail') {
        ...
        // Branch user: deny access to shifts that don't belong to their terminal
        if (empty($user->admin)) {
            require_once DOL_DOCUMENT_ROOT . '/takepos/class/TakeposBranchService.class.php';
            $branch = TakeposBranchService::getBranchByUserId($db, (int) $user->id);
            if ($branch) {
                $branchTerminals = TakeposBranchService::getTerminalsForBranchId($db, (int) $branch->rowid);
                $branchTerminalIds = array_keys($branchTerminals);
                if (!in_array((int) $shift->fk_terminal, $branchTerminalIds, true)) {
                    takeposShiftJson(array('success' => false, 'error' => 'access_denied', 'message' => 'Access denied'), 403);
                }
            }
        }
