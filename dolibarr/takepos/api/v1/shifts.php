...
if ($shiftId > 0) {
    ...
    takeposApiAuditAccess($db, $auth, 'shifts.show', array('shift_id' => $shiftId));
    takeposApiSuccess(takeposApiShiftPayload($row), array('entity' => $entity));
}

// ── List shifts ───────────────────────────────────────────────────────────────
$rows = array();
foreach (TakeposShiftService::listShifts($db, $entity, $filters, $limit) as $row) {
    $rows[] = takeposApiShiftPayload($row);
}

takeposApiAuditAccess($db, $auth, 'shifts.index', array('filters' => $filters, 'count' => count($rows)));
