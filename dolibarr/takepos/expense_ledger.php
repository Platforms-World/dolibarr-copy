...
$pageUrl = DOL_URL_ROOT . '/takepos/expense_ledger.php';
...
TakeposAccess::requireFrontendAccess(
    ...
    array('page' => 'expense_ledger.php')
);

if (!TakeposExpenseService::canRead($db, $user)) {
    TakeposAccess::denyAccess($db, $user, $langs->trans('TakeposExpenseReadPermissionRequired'), array('page' => 'expense_ledger.php', 'permission' => 'takepos.expense.read'));
}

TakeposAudit::logEvent($db, $user, 'expense_ledger_opened', TakeposAudit::SEVERITY_INFO, array('page' => 'expense_ledger.php'), 'POS expense ledger opened');
...
<div class="takepos-workspace-reports-page">
    <div class="takepos-workspace-title-row">
        <h2 class="takepos-workspace-title"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerTitle')); ?></h2>
        <div class="takepos-workspace-filter-actions takepos-workspace-ledger-top-actions">
            <?php if ($canCreateExpense) { ?>
                <a class="button button-save" href="<?php echo dol_escape_htmltag(DOL_URL_ROOT . '/takepos/workspace.php?key=expenses_ops'); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerNewExpense')); ?></a>
            <?php } ?>
            <?php if ($canAdminExpense) { ?>
                <a class="button" href="<?php echo dol_escape_htmltag(DOL_URL_ROOT . '/takepos/workspace.php?key=admin_expense_categories'); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerExpenseCategories')); ?></a>
            <?php } ?>
            <a class="button" href="<?php echo dol_escape_htmltag($pageUrl . '?' . http_build_query(array_merge($baseQuery, array('action' => 'export_csv')))); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerExportCsv')); ?></a>
            <a class="button button-cancel" href="<?php echo dol_escape_htmltag($pageUrl); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerResetFilters')); ?></a>
        </div>
    </div>
    ...
    <section class="takepos-workspace-panel takepos-workspace-filter-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerFilters')); ?></h3>
        <form method="get" action="<?php echo dol_escape_htmltag($pageUrl); ?>">
            ...
            <div class="takepos-workspace-filter-actions">
                <button type="submit" class="button button-save"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerApplyFilters')); ?></button>
                <a class="button button-cancel" href="<?php echo dol_escape_htmltag($pageUrl); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonReset')); ?></a>
            </div>
    ...
    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerSummary')); ?></h3>
        <div class="takepos-workspace-summary-cards">
            <div class="takepos-workspace-card"><div class="takepos-workspace-card-label"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerTotalCount')); ?></div><div class="takepos-workspace-card-value"><?php echo (int) (!empty($summary->total_count) ? $summary->total_count : 0); ?></div></div>
            <div class="takepos-workspace-card"><div class="takepos-workspace-card-label"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerTotalHt')); ?></div><div class="takepos-workspace-card-value"><?php echo price((float) (!empty($summary->total_amount_ht) ? $summary->total_amount_ht : 0)); ?></div></div>
            <div class="takepos-workspace-card"><div class="takepos-workspace-card-label"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerTotalVat')); ?></div><div class="takepos-workspace-card-value"><?php echo price((float) (!empty($summary->total_amount_tva) ? $summary->total_amount_tva : 0)); ?></div></div>
            <div class="takepos-workspace-card"><div class="takepos-workspace-card-label"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerTotalTtc')); ?></div><div class="takepos-workspace-card-value"><?php echo price((float) (!empty($summary->total_amount_ttc) ? $summary->total_amount_ttc : 0)); ?></div></div>
            <div class="takepos-workspace-card"><div class="takepos-workspace-card-label"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerPostedCount')); ?></div><div class="takepos-workspace-card-value"><?php echo (int) (!empty($summary->posted_count) ? $summary->posted_count : 0); ?></div></div>
            <div class="takepos-workspace-card"><div class="takepos-workspace-card-label"><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerNotPostedCount')); ?></div><div class="takepos-workspace-card-value"><?php echo (int) (!empty($summary->not_posted_count) ? $summary->not_posted_count : 0); ?></div></div>
        </div>
    </section>

    <section class="takepos-workspace-panel">
        <div class="takepos-workspace-ledger-headerline">
            <h3><?php echo dol_escape_htmltag($langs->trans('TakeposExpenseLedgerTitle')); ?></h3>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="takepos-ledger-actions">
                                    <a class="button" href="<?php echo dol_escape_htmltag(DOL_URL_ROOT . '/takepos/expenses.php?id=' . ((int) $row->rowid)); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonOpen')); ?></a>
                                    <?php if (TakeposExpenseService::canPostExpenseRecord($db, $user, $row)) { ?>
                                        <form method="post" action="<?php echo dol_escape_htmltag($pageUrl . '?' . http_build_query(array_merge($baseQuery, array('page' => $page)))); ?>" class="takepos-inline-form">
                                            ...
                                            <input type="hidden" name="expense_id" value="<?php echo (int) $row->rowid; ?>">
                                            <button type="submit" class="button button-save" onclick="return confirm('<?php echo dol_escape_js($langs->trans('TakeposExpenseLedgerPostConfirm')); ?>');"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonPost')); ?></button>
                                        </form>
                                    <?php } ?>
                                </div>
                            </td>
        ...
        <?php if ($totalPages > 1) { ?>
            <div class="takepos-workspace-filter-actions takepos-ledger-pagination">
                <?php if ($page > 1) { ?>
                    <a class="button" href="<?php echo dol_escape_htmltag(takeposExpenseLedgerUrl($pageUrl, array_merge($baseQuery, array('page' => $page - 1)))); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonPrevious')); ?></a>
                <?php } ?>
                ...
                <?php if ($page < $totalPages) { ?>
                    <a class="button" href="<?php echo dol_escape_htmltag(takeposExpenseLedgerUrl($pageUrl, array_merge($baseQuery, array('page' => $page + 1)))); ?>"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonNext')); ?></a>
                <?php } ?>
            </div>
        <?php } ?>
    </section>
