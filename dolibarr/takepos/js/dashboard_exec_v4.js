...
(function () {
    ...
    function render(data) {
        ...
        renderLowStockAlerts(data.inventory_alerts || []);
    }
    // FIX (stock-branch-v5): Render low-stock product list with "Create Purchase" buttons.
    // inventory_alerts was returned by the dashboard API but never displayed in the UI.
    function renderLowStockAlerts(alerts) {
        var host = q('tp_low_stock_alerts');
        if (!host) return;
        var labels = window.takeposDashboardLabels || {};
        if (!alerts || alerts.length === 0) {
            host.innerHTML = '<p style="color:#6b7280;font-size:12px;margin:0">' + escapeHtml(labels.LowStockNone || 'All products are above their minimum threshold.') + '</p>';
            return;
        }
        var purchaseUrl = labels.PurchaseUrl || '';
        var html = '<table style="width:100%;border-collapse:collapse;font-size:12px">';
        html += '<thead><tr>'
            + '<th style="text-align:left;padding:5px 6px;border-bottom:1px solid #e5e7eb;color:#374151">' + escapeHtml(labels.RefLabel || 'Ref') + '</th>'
            + '<th style="text-align:left;padding:5px 6px;border-bottom:1px solid #e5e7eb;color:#374151">' + escapeHtml(labels.ProductLabel || 'Product') + '</th>'
            + '<th style="text-align:right;padding:5px 6px;border-bottom:1px solid #e5e7eb;color:#374151">' + escapeHtml(labels.LowStockLabel || 'Stock') + '</th>'
            + '<th style="text-align:right;padding:5px 6px;border-bottom:1px solid #e5e7eb;color:#374151">Min</th>'
            + '<th style="padding:5px 6px;border-bottom:1px solid #e5e7eb"></th>'
            + '</tr></thead><tbody>';
        alerts.forEach(function(a) {
            var stock = parseFloat(a.stock) || 0;
            var threshold = parseFloat(a.threshold) || 5;
            var stockColor = stock <= 0 ? '#b91c1c' : '#b45309';
            // Suggested reorder = threshold - current + buffer (one more threshold cycle)
            var suggestedQty = Math.max(1, Math.ceil(threshold - stock + threshold));
            var href = purchaseUrl
                + '&prefill_product_id=' + encodeURIComponent(a.product_id || '')
                + '&prefill_qty=' + encodeURIComponent(suggestedQty);
            html += '<tr>'
                + '<td style="padding:5px 6px;border-bottom:1px solid #f3f4f6;font-weight:700">' + escapeHtml(a.ref) + '</td>'
                + '<td style="padding:5px 6px;border-bottom:1px solid #f3f4f6">' + escapeHtml(a.label) + '</td>'
                + '<td style="padding:5px 6px;border-bottom:1px solid #f3f4f6;text-align:right;font-weight:700;color:' + stockColor + '">' + stock + '</td>'
                + '<td style="padding:5px 6px;border-bottom:1px solid #f3f4f6;text-align:right;color:#6b7280">' + threshold + '</td>'
                + '<td style="padding:5px 6px;border-bottom:1px solid #f3f4f6;text-align:right">'
                + (purchaseUrl ? '<a href="' + href + '" style="display:inline-block;padding:3px 8px;background:#1d4ed8;color:#fff;border-radius:5px;font-size:11px;font-weight:700;text-decoration:none">+ ' + escapeHtml(labels.CreatePurchase || 'Order') + '</a>' : '')
                + '</td></tr>';
        });
        html += '</tbody></table>';
        host.innerHTML = html;
