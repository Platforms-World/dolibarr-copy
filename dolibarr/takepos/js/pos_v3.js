...
(function () {
  ...
  /* Topbar + Rail lang buttons */
  function kfSwitchLang() {
    var url = C.isArabic ? C.langUrlEn : C.langUrlAr;
    if (url) window.location.href = url;
  }
  var btnTopLang = document.getElementById('kfTopbarLang');
  if (btnTopLang) btnTopLang.addEventListener('click', kfSwitchLang);

  /* Lang button */
  var btnLang = document.getElementById('kfRailLang');
  if (btnLang) btnLang.addEventListener('click', function () {
    var cur = document.documentElement.lang || 'ar';
    var isAr = cur.indexOf('ar') === 0;
    window.location.href = (window.KAFO && window.KAFO.ajaxUrl
      ? window.KAFO.ajaxUrl.replace('ajax/ajax.php','ajax/lang_switch.php')
      : '/takepos/ajax/lang_switch.php')
      + '?lang=' + (isAr ? 'en_US' : 'ar_JO')
