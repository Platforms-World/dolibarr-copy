...
TakeposAccess::enforceAdmin($db, $user, 'takepos.admin.setup', null);
...
if ($action == 'set') {
    ...
    // TakePOS Expiry warnings
    $expiryWarn = (int) GETPOST('TAKEPOS_EXPIRY_WARN_DAYS', 'int');
    if ($expiryWarn < 1) $expiryWarn = 30;
    $res = dolibarr_set_const($db, "TAKEPOS_EXPIRY_WARN_DAYS", $expiryWarn, 'chaine', 0, '', $conf->entity);
    $expiryCrit = (int) GETPOST('TAKEPOS_EXPIRY_CRIT_DAYS', 'int');
    if ($expiryCrit < 1) $expiryCrit = 7;
    $res = dolibarr_set_const($db, "TAKEPOS_EXPIRY_CRIT_DAYS", $expiryCrit, 'chaine', 0, '', $conf->entity);
...
// Show product images in POS screen
print '<tr class="oddeven"><td>';
print $form->textwithpicto('عرض صور المنتجات', 'عند التفعيل تظهر صورة المنتج داخل بطاقة المنتج في شاشة البيع. عند الإيقاف تُخفى الصور ويُعرض المنتج بتنسيق نصي مدمج.', 1, 'help', '', 0, 3, 'showproductimages');
print '<td>';
print ajax_constantonoff("TAKEPOS_SHOW_PRODUCT_IMAGES", array(), $conf->entity, 0, 0, 1, 0);
print "</td></tr>\n";

// Print preview toggle
print '<tr class="oddeven"><td>';
print $form->textwithpicto('معاينة الطباعة قبل الطباعة', 'عند التفعيل تظهر نافذة معاينة الإيصال قبل الإرسال للطابعة. عند الإيقاف تُرسل مباشرة للطابعة الافتراضية بدون تدخل الكاشير.', 1, 'help', '', 0, 3, 'printpreview');
print '<td>';
print ajax_constantonoff("TAKEPOS_PRINT_PREVIEW", array(), $conf->entity, 0, 0, 1, 0);
print "</td></tr>\n";
...
// ── TakePOS: إعدادات تنبيه انتهاء الصلاحية ─────────────────────────────────
print '<tr class="liste_titre">';
print '<td colspan="2" style="padding:12px 8px 8px">';
print '<span style="font-size:14px;font-weight:700">⏰ '.($langs->trans('TakeposExpiryAlertSettings') !== 'TakeposExpiryAlertSettings' ? $langs->trans('TakeposExpiryAlertSettings') : 'إعدادات تنبيه انتهاء الصلاحية').'</span>';
print '</td></tr>';
print '<tr class="oddeven">';
print '<td>';
print '<b>'.($langs->trans('TakeposExpiryWarnDays') !== 'TakeposExpiryWarnDays' ? $langs->trans('TakeposExpiryWarnDays') : 'أيام التحذير المبكر').'</b>';
print '<br><span class="opacitymedium" style="font-size:11px">'.($langs->trans('TakeposExpiryWarnDaysDesc') !== 'TakeposExpiryWarnDaysDesc' ? $langs->trans('TakeposExpiryWarnDaysDesc') : 'عدد الأيام قبل الانتهاء لإظهار التحذير البرتقالي في POS').'</span>';
print '</td>';
print '<td>';
print '<input type="number" name="TAKEPOS_EXPIRY_WARN_DAYS" min="1" max="365" style="width:70px;padding:4px 8px;border:1px solid #ccc;border-radius:6px;font-size:14px;font-weight:700;text-align:center"';
print ' value="'.getDolGlobalInt('TAKEPOS_EXPIRY_WARN_DAYS', 30).'">';
print ' <span style="display:inline-block;padding:3px 12px;border-radius:20px;background:#ea580c;color:#fff;font-size:12px;font-weight:700;margin-left:8px;vertical-align:middle">⚠ برتقالي</span>';
print '</td></tr>';
print '<tr class="oddeven">';
print '<td>';
print '<b>'.($langs->trans('TakeposExpiryCritDays') !== 'TakeposExpiryCritDays' ? $langs->trans('TakeposExpiryCritDays') : 'أيام التحذير الحرج').'</b>';
print '<br><span class="opacitymedium" style="font-size:11px">'.($langs->trans('TakeposExpiryCritDaysDesc') !== 'TakeposExpiryCritDaysDesc' ? $langs->trans('TakeposExpiryCritDaysDesc') : 'عدد الأيام قبل الانتهاء لإظهار التحذير الأحمر المومض في POS').'</span>';
print '</td>';
print '<td>';
print '<input type="number" name="TAKEPOS_EXPIRY_CRIT_DAYS" min="1" max="365" style="width:70px;padding:4px 8px;border:1px solid #ccc;border-radius:6px;font-size:14px;font-weight:700;text-align:center"';
print ' value="'.getDolGlobalInt('TAKEPOS_EXPIRY_CRIT_DAYS', 7).'">';
print ' <span style="display:inline-block;padding:3px 12px;border-radius:20px;background:#dc2626;color:#fff;font-size:12px;font-weight:700;margin-left:8px;vertical-align:middle">🔴 أحمر يومض</span>';
print '</td></tr>';
// ── END TakePOS Expiry Settings ──────────────────────────────────────────────
