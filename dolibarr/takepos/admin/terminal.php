...
// FIX (M7): Removed direct writes to $_GET/$_POST/$_REQUEST superglobals.
// A naming mismatch between the HTML field (CASHDESK_ID_THIRDPARTY{N}_id)
// and the constant (CASHDESK_ID_THIRDPARTY{N}) was patched by mutating
// superglobals, which bypasses Dolibarr input sanitization. Now captured
// into a local variable and used directly in dolibarr_set_const below.
$_tpThirdpartyOverride = (int) GETPOST('CASHDESK_ID_THIRDPARTY'.$terminal.'_id', 'alpha');
...
TakeposAccess::enforceAdmin($db, $user, 'takepos.admin.terminal', $terminaltouse);
...
if (GETPOST('action', 'alpha') == 'set') {
	...
	// FIX (M7 cont): use $_tpThirdpartyOverride from selector, fallback to socid.
	$_tpThirdpartyFinal = $_tpThirdpartyOverride > 0 ? $_tpThirdpartyOverride : (GETPOSTINT('socid') > 0 ? GETPOSTINT('socid') : '');
	$res = dolibarr_set_const($db, "CASHDESK_ID_THIRDPARTY".$terminaltouse, $_tpThirdpartyFinal, 'chaine', 0, '', $conf->entity);
	...
	// Terminal PIN (secondary password for POS access)
	if (GETPOSTISSET('TAKEPOS_TERMINAL_PIN_'.$terminaltouse)) {
		$pinRaw = GETPOST('TAKEPOS_TERMINAL_PIN_'.$terminaltouse, 'alphanohtml');
		if ($pinRaw !== '') {
			$res = dolibarr_set_const($db, 'TAKEPOS_TERMINAL_PIN_'.$terminaltouse, password_hash($pinRaw, PASSWORD_DEFAULT), 'chaine', 0, '', $conf->entity);
		} elseif (GETPOST('TAKEPOS_TERMINAL_PIN_CLEAR_'.$terminaltouse, 'alpha') === '1') {
			$res = dolibarr_set_const($db, 'TAKEPOS_TERMINAL_PIN_'.$terminaltouse, '', 'chaine', 0, '', $conf->entity);
		}
	}
	// PIN length (global setting)
	if (GETPOSTISSET('TAKEPOS_TERMINAL_PIN_LENGTH')) {
		$pinLen = max(4, min(8, GETPOSTINT('TAKEPOS_TERMINAL_PIN_LENGTH')));
		$res = dolibarr_set_const($db, 'TAKEPOS_TERMINAL_PIN_LENGTH', $pinLen, 'chaine', 0, '', $conf->entity);
	}
...
if (isModEnabled("bank")) {
	...
	print ' <a href="'.DOL_URL_ROOT.'/compta/bank/card.php?action=create&token='.newToken().'&type=2&backtopage='.urlencode($_SERVER["PHP_SELF"].'?terminal='.$terminal).'"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("NewBankAccount").'"></span></a>';
	...
	print ' <a href="'.DOL_URL_ROOT.'/compta/bank/card.php?action=create&token='.newToken().'&type=1&backtopage='.urlencode($_SERVER["PHP_SELF"].'?terminal='.$terminal).'"><span class="fa fa-plus-circle valignmiddle paddingleft" title="'.$langs->trans("NewBankAccount").'"></span></a>';
...
if (isModEnabled('stock')) {
	...
	if (!$disabled) {
		...
		print ' <a href="'.DOL_URL_ROOT.'/product/stock/card.php?action=create&token='.newToken().'&backtopage='.urlencode($_SERVER["PHP_SELF"].'?&terminal='.$terminal).'"><span class="fa fa-plus-circle valignmiddle"></span></a>';
	} else {
...
$pinSet = !empty(getDolGlobalString('TAKEPOS_TERMINAL_PIN_'.$terminaltouse));
print '<tr class="oddeven"><td>';
print '<b>Terminal PIN</b>';
print ' <span class="opacitymedium">&mdash; Secondary password required to open this terminal in POS</span>';
print '</td>';
print '<td>';
if ($pinSet) {
	print '<span style="color:green;font-weight:bold">&#10003; PIN is set</span>';
	print ' &nbsp; <label style="font-size:0.9em"><input type="checkbox" name="TAKEPOS_TERMINAL_PIN_CLEAR_'.$terminaltouse.'" value="1"> Remove PIN</label>';
	print '<br><span style="font-size:0.9em;color:#666">Set new PIN to replace:</span> ';
} else {
	print '<span style="color:#c0392b">No PIN set (terminal open to all)</span><br>';
}
print '<input type="password" class="width100" name="TAKEPOS_TERMINAL_PIN_'.$terminaltouse.'" placeholder="'.($pinSet ? 'New PIN (leave empty to keep)' : 'Set PIN').'" autocomplete="new-password" />';
print '</td></tr>';

// PIN length (global)
print '<tr class="oddeven"><td>PIN Length <span class="opacitymedium">&mdash; Number of digits required (4-8, applies to all terminals)</span></td>';
print '<td>';
print '<select name="TAKEPOS_TERMINAL_PIN_LENGTH" class="flat">';
for ($l = 4; $l <= 8; $l++) {
	$sel = (getDolGlobalInt('TAKEPOS_TERMINAL_PIN_LENGTH', 4) == $l) ? ' selected' : '';
	print '<option value="'.$l.'"'.$sel.'>'.$l.' digits</option>';
}
print '</select>';
print '</td></tr>';
