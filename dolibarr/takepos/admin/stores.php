...
if (isModEnabled('stock')) {
    // FIX (warehouse-dropdown-v2): see branches.php — drop status filter
    // (column name varies across Dolibarr versions) AND broaden entity scope
    // (multi-company installs may store warehouses in a different entity).
    $sqlWh = "SELECT rowid, ref, label FROM " . MAIN_DB_PREFIX . "entrepot WHERE entity IN (" . getEntity('stock') . ") ORDER BY ref ASC";
