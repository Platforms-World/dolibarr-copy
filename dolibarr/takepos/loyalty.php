...
TakeposAccess::requireFrontendAccess($db, $user, 'takepos.crm', 'takepos.customer.view', $terminal, $langs->trans('TakeposLoyaltyAccessDenied'), array('page' => 'loyalty.php'));
...
<div class="takepos-workspace-reports-page">
    <h2 class="takepos-workspace-title"><?php echo htmlspecialchars($loyaltyTitlePlain, ENT_QUOTES, 'UTF-8'); ?></h2>

    <div id="takepos-loyalty-config"
         data-token="<?php echo dol_escape_htmltag(newToken()); ?>"
         data-endpoint="<?php echo dol_escape_htmltag(DOL_URL_ROOT . '/takepos/ajax/loyalty.php'); ?>"
         data-can-admin="<?php echo (!empty($user->admin) ? '1' : '0'); ?>"></div>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyCustomerLookup')); ?></h3>
        <div class="takepos-workspace-filter-grid">
            ...
            <div style="align-self:end;"><button type="button" id="btn_lookup" class="button button-save"><?php echo dol_escape_htmltag($langs->trans('TakeposCommonSearch')); ?></button></div>
        </div>
        <div class="takepos-workspace-table-wrap"><table id="table_customers" class="takepos-workspace-table"></table></div>
    </section>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyCustomerSummary')); ?></h3>
        <div id="loyalty_summary_cards" class="takepos-workspace-summary-cards"></div>
    </section>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyActions')); ?></h3>
        <div class="takepos-workspace-filter-grid">
            ...
            <div style="align-self:end;"><button type="button" id="btn_redeem" class="button button-save"><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyRedeemOnInvoice')); ?></button></div>
            ...
            <div style="align-self:end;"><button type="button" id="btn_adjust" class="button"><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyAdjustPointsAction')); ?></button></div>
        </div>
        <div id="loyalty_message" class="opacitymedium" style="margin-top:8px;"></div>
    </section>

    <section class="takepos-workspace-panel" id="settings_panel" style="display:none;">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltySettings')); ?></h3>
        <div class="takepos-workspace-filter-grid">
            ...
            <div style="align-self:end;"><button type="button" id="btn_save_settings" class="button"><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltySaveSettings')); ?></button></div>
        </div>
    </section>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyRecentTickets')); ?></h3>
        <div class="takepos-workspace-table-wrap"><table id="table_tickets" class="takepos-workspace-table"></table></div>
    </section>

    <section class="takepos-workspace-panel">
        <h3><?php echo dol_escape_htmltag($langs->trans('TakeposLoyaltyTransactions')); ?></h3>
        <div class="takepos-workspace-table-wrap"><table id="table_txn" class="takepos-workspace-table"></table></div>
    </section>
</div>
...
(function () {
    ...
    function renderSummary(summary) {
        ...
        if (!summary || !summary.customer || !summary.loyalty) {
            cards.innerHTML = '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.customer) + '</div><div class="takepos-workspace-card-value">' + safe(i18n.selectCustomer) + '</div></div>';
            return;
        }

        cards.innerHTML = ''
            + '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.customer) + '</div><div class="takepos-workspace-card-value">' + safe(summary.customer.name) + '</div></div>'
            + '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.pointsBalance) + '</div><div class="takepos-workspace-card-value">' + safe(summary.loyalty.points_balance) + '</div></div>'
            + '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.totalEarned) + '</div><div class="takepos-workspace-card-value">' + safe(summary.loyalty.total_earned) + '</div></div>'
            + '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.totalRedeemed) + '</div><div class="takepos-workspace-card-value">' + safe(summary.loyalty.total_redeemed) + '</div></div>'
            + '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.purchaseCount) + '</div><div class="takepos-workspace-card-value">' + safe(summary.purchase ? summary.purchase.purchase_count : 0) + '</div></div>'
            + '<div class="takepos-workspace-card"><div class="takepos-workspace-card-label">' + safe(i18n.grossSales) + '</div><div class="takepos-workspace-card-value">' + fmt(summary.purchase ? summary.purchase.gross_sales : 0) + '</div></div>';
    }
