...
TakeposAccess::enforceFrontend($db, isset($user) ? $user : null, 'takepos.smpcb', $_SESSION["takeposterminal"]);

if (GETPOSTISSET('status')) {
	if (!headers_sent()) {
		header('Content-Type: text/plain; charset=UTF-8');
	}
	echo strtoupper((string) (isset($_SESSION['SMP_CURRENT_PAYMENT']) ? $_SESSION['SMP_CURRENT_PAYMENT'] : ''));
	exit;
}
