...
TakeposAccess::enforceFrontend($db, isset($user) ? $user : null, 'takepos.freezone', $_SESSION["takeposterminal"]);
...
if ($action == "freezone" && $user->hasRight('takepos', 'run')) {
	echo '<input type="number" step="0.001" min="-999999999" id="number" name="number" class="takepospay" style="width:15%;" placeholder="'.$langs->trans(getDolGlobalString("TAKEPOS_CHANGE_PRICE_HT") ? 'AmountHT' : 'AmountTTC').'">';
}
