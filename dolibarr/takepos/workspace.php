...
$map = array(
    ...
    'add_product' => array(
        ...
        'url' => DOL_URL_ROOT.'/product/card.php?action=create&token='.newToken().'&type=0'
            // FIX: pass CASHDESK_ID_WAREHOUSE1 so product/card.php pre-selects
            // the terminal's warehouse in the product form. Without this,
            // fk_default_warehouse is NULL on new products, causing the
            // "Correct Stock" form to default to the wrong warehouse and
            // the POS to show "? Stock" even after stock is added.
            .(getDolGlobalInt('CASHDESK_ID_WAREHOUSE1') > 0
                ? '&fk_default_warehouse='.getDolGlobalInt('CASHDESK_ID_WAREHOUSE1')
                : ''),
    ...
    'barcode_scale' => array(
        'feature' => 'takepos.use',
        'url'     => DOL_URL_ROOT.'/takepos/admin/barcode_scale.php',
        'check'   => function($user) { return !empty($user->admin); },
    ...
    'admin_branches' => array(
        ...
        'admin' => true,
        'message' => 'Access denied – Branch Management is restricted to the main administrator only | إدارة الفروع متاحة للمسؤول الرئيسي فقط'
    ),
    ...
    // نظام الفوترة الإقليمية (الأردن / المملكة العربية السعودية ZATCA)
    'admin_billing_country' => array(
        'feature' => 'takepos.admin.receipt',
        'url' => DOL_URL_ROOT.'/takepos/admin/billing_country.php',
        'admin' => true,
        'message' => 'يجب أن تكون مشرفاً للوصول إلى إعدادات نظام الفوترة'
    ),
