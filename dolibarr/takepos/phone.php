...
$langs->loadLangs(array("bills", "orders", "commercial", "cashdesk", "receiptprinter", "takeposcustom@takepos"));

if (!$user->hasRight('takepos', 'run') && !defined('INCLUDE_PHONEPAGE_FROM_PUBLIC_PAGE')) {
	accessforbidden($langs->trans('TakeposPhoneAccessDenied'));
}

TakeposAccess::enforceFrontend($db, isset($user) ? $user : null, 'takepos.frontend', $_SESSION["takeposterminal"]);
