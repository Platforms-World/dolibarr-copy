...
if ($action === 'hold') {
    ...
    TakeposAudit::logEvent(
        ...
        array('invoice_id' => $invoiceid, 'terminal' => $terminal, 'label' => $label),
        'Sale held/suspended'
    );
...
if ($action === 'resume') {
    ...
    // Fetch hold record strictly for the current terminal and shift/user scope
    $sqlGet = "SELECT h.rowid, h.fk_invoice, h.fk_terminal"
        ...
        . " AND h.status = 1";
    if ($hasShiftColumn) {
        $sqlGet .= " AND h.fk_shift = " . ((int) $activeShiftId);
    } else {
        $sqlGet .= " AND h.fk_user = " . ((int) $user->id);
    }
    ...
    if ($res <= 0 || $invoice->status != Facture::STATUS_DRAFT) {
        // Mark hold as cancelled if invoice gone/validated
        $db->query("UPDATE " . MAIN_DB_PREFIX . "takepos_held_sale SET status=0, date_update='" . $db->idate(dol_now()) . "' WHERE rowid=" . ((int) $holdid));
        holdJsonOut(array('success' => false, 'error' => 'Held invoice no longer available'));
    }
    ...
    // Extract place from the held invoice ref: (PROV-POS{term}-{place})
    $heldPlace = 0;
    ...
    TakeposAudit::logEvent(
        ...
        'pos_resume_sale',
        TakeposAudit::SEVERITY_INFO,
        array('invoice_id' => $heldInvoiceId, 'terminal' => $terminal),
        'Held sale resumed'
    );
