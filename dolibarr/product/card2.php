        ...
        if (!$error) {
            ...
            if ($id > 0) {
                ...
                if (isModEnabled('takepos')) {
                    $tpbcBarcodes = isset($_POST['tpbc_barcode']) && is_array($_POST['tpbc_barcode']) ? $_POST['tpbc_barcode'] : [];
                    if (!empty($tpbcBarcodes)) {
                        $tpbcLabels = isset($_POST['tpbc_label']) && is_array($_POST['tpbc_label']) ? $_POST['tpbc_label'] : [];
                        ...
                        if ($tpbcChk && (int) $db->num_rows($tpbcChk) > 0) {
                            foreach ($tpbcBarcodes as $tpbcIdx => $tpbcBc) {
                                $tpbcBc  = trim((string) $tpbcBc);
                                $tpbcLbl = trim((string) ($tpbcLabels[$tpbcIdx] ?? ''));
                                if ($tpbcBc === '') continue;
                                $db->query("INSERT IGNORE INTO " . $tpbcTable . " (fk_product, barcode, label, entity) VALUES (" . (int) $id . ", '" . $db->escape($tpbcBc) . "', '" . $db->escape($tpbcLbl) . "', " . (int) $conf->entity . ")");
                            }
                        }
                    }
                }
...
if (isModEnabled('takepos') && ((!empty($object->id) && $object->id > 0) || $action === 'create')) {
        ...
        (function(){
            ...
            jQuery(function($){
                try{
                    ...
                    var $wh=n('fk_default_warehouse').closest('td').children().first();
                    var $desc=$form.find('[name=desc]').first().closest('td').children().first();
                    ...
                    /* barcode type */
                    if($barcType.length){$barcType.css('width','180px');$card.append(row('Barcode type',$barcType.detach()));}
                    ...
                    /* warehouse */
                    if($wh.length){$wh.css('width','100%');$card.append(row('Warehouse',$wh.detach()));}

                    /* description */
                    if($desc.length){$desc.addClass('pc-desc-ta');$card.append(row('Description',$desc.detach(),true));}
                    ...
                    /* ── Extra barcodes card ── */
                    var $xbc=$('<div class="pc-xbc-card">'+
                        '<div class="pc-xbc-hd">'+
                        '<svg width="17" height="17" viewBox="0 0 24 24"><rect x="2" y="4" width="3" height="16" fill="#1a56db"/><rect x="7" y="4" width="1" height="16" fill="#1a56db"/><rect x="10" y="4" width="2.5" height="16" fill="#1a56db"/><rect x="15" y="4" width="1" height="16" fill="#1a56db"/><rect x="18" y="4" width="4" height="16" fill="#1a56db"/></svg>'+
                        '<span class="pc-xbc-title">Extra Barcodes</span>'+
                        '<span class="pc-xbc-cnt" id="pc-xbc-n">0</span>'+
                        '<span class="pc-xbc-sub">TakePos POS scan</span>'+
                        '</div>'+
                        '<div class="pc-xbc-body">'+
                        '<div class="pc-xbc-hint">'+
                        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#4285f4" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>'+
                        'Scan or type extra barcodes. All searchable from POS. <strong>Save product first</strong> — barcodes stored automatically.'+
                        '</div>'+
                        '<div class="pc-xbc-inputs">'+
                        '<input class="pc-xbc-val" id="pc-xv" placeholder="Barcode value (scan / type)">'+
                        '<input class="pc-xbc-lbl" id="pc-xl" placeholder="Label (optional)">'+
                        '<button type="button" class="pc-xbc-add" id="pc-xbc-add-btn">+ Add</button>'+
                        '</div>'+
                        '<div class="pc-xbc-tags" id="pc-xt"></div>'+
                        '<div class="pc-xbc-empty" id="pc-xe">No extra barcodes yet</div>'+
                        '</div>'+
                        '</div>');
                    $left.append($xbc);
                    ...
                    var xbcs=[];
                    function renderXbc(){
                        $('#pc-xbc-n').text(xbcs.length);
                        var $t=$('#pc-xt');$t.empty();
                        $form.find('input[name="tpbc_barcode[]"],input[name="tpbc_label[]"]').remove();
                        xbcs.forEach(function(b,i){
                            var $tag=$('<span class="pc-xbc-tag"></span>');
                            if(b.l)$tag.append('<b style="color:#1a56db">'+b.l+'</b>: ');
                            $tag.append(document.createTextNode(b.v));
                            $tag.append('<span class="pc-xbc-del" data-i="'+i+'">×</span>');
                            $t.append($tag);
                            $form.append('<input type="hidden" name="tpbc_barcode[]" value="'+b.v+'">');
                            $form.append('<input type="hidden" name="tpbc_label[]" value="'+b.l+'">');
                        });
                        $('#pc-xe').toggle(xbcs.length===0);
                    }
                    $(document).on('click','.pc-xbc-del',function(){xbcs.splice(parseInt($(this).data('i')),1);renderXbc();});
                    $('#pc-xbc-add-btn').on('click',function(){
                        var v=$.trim($('#pc-xv').val());if(!v)return;
                        xbcs.push({v:v,l:$.trim($('#pc-xl').val())});renderXbc();
                        $('#pc-xv').val('').focus();$('#pc-xl').val('');
                    });
                    $('#pc-xv').on('keydown',function(e){if(e.key==='Enter'){e.preventDefault();$('#pc-xbc-add-btn').click();}});
                    ...
                    if($price.length){
                        ...
                        if($priceBase.length)$pbWrap.append($priceBase.detach());
                    ...
                    if($tva.length){$tva.css('width','100%');$pricing.append(srow('VAT rate',$tva.detach()));}
                    ...
                    if($batch.length){$batch.css('width','100%');$stock.append(srow('Lot / serial',$batch.detach()));}
                    $right.append($stock);
                    ...
                    if($nativeCat.length){$nativeCat.css('width','100%');$catWrap.append($nativeCat.detach());}
                    ...
                    if($accSell.length){$accSell.css('width','100%');$acc.append(srow('Sell code',$accSell.detach()));}
                    if($accBuy.length){$accBuy.css('width','100%');$acc.append(srow('Buy code',$accBuy.detach()));}
