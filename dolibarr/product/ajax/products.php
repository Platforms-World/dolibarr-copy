...
if ($action == 'fetch' && !empty($id)) {
	...
	if (GETPOST($htmlname, 'nohtml') == '' && (!$idprod || !GETPOST($idprod, 'nohtml'))) {
		print json_encode(array());
		return;
	}

	// When used from jQuery, the search term is added as GET param "term".
	$searchkey = (($idprod && GETPOST($idprod, 'nohtml')) ? GETPOST($idprod, 'nohtml') : (GETPOST($htmlname, 'nohtml') ? GETPOST($htmlname, 'nohtml') : ''));
