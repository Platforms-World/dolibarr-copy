 ...
 * Copyright (C) 2019	Josep Lluأ­s Amador	<joseplluis@lliuretic.cat>
 * Copyright (C) 2020	Thibault FOUCART	<support@ptibogxiv.net>
 * Copyright (C) 2024-2025	MDW				<mdeweerd@users.noreply.github.com>
 * Copyright (C) 2024       Frأ©dأ©ric France         <frederic.france@free.fr>
 *
...
$isSaasFeatureEnabled = function ($featureCode) use ($db) {
	return TakeposSaasBridge::isFeatureEnabled($db, $featureCode);
};
$canCreateProducts = ($user->admin || $user->hasRight('produit', 'creer') || $user->hasRight('service', 'creer'));
$canReadProducts = ($user->admin || $user->hasRight('produit', 'lire') || $user->hasRight('service', 'lire'));
$canCreateCategories = ($user->admin || $user->hasRight('categorie', 'creer'));
$canReadCategories = ($user->admin || $user->hasRight('categorie', 'lire'));

if ($canCreateProducts && $isSaasFeatureEnabled('takepos.catalog.add_product')) {
	$productStudioLinks[] = array(
		'group' => 'create',
		'label' => 'Add Product',
		'description' => 'Create a new sellable product card',
		'icon' => 'fa fa-box',
		'url' => '/product/card.php?action=create&type=0',
		'feature' => 'takepos.catalog.add_product'
	);
}
if ($canCreateProducts && $isSaasFeatureEnabled('takepos.catalog.add_service')) {
	$productStudioLinks[] = array(
		'group' => 'create',
		'label' => 'Add Service',
...
if ($canReadProducts && $isSaasFeatureEnabled('takepos.catalog.manage_products')) {
	$productStudioLinks[] = array(
		'group' => 'manage',
		'label' => 'Manage Products',
		'description' => 'Open product list and filters',
		'icon' => 'fa fa-th-list',
		'url' => '/product/list.php?type=0',
		'feature' => 'takepos.catalog.manage_products'
	);
}
if ($canReadProducts && $isSaasFeatureEnabled('takepos.catalog.manage_services')) {
	$productStudioLinks[] = array(
		'group' => 'manage',
		'label' => 'Manage Services',
		'description' => 'Open services list and filters',
		'icon' => 'fa fa-stream',
		'url' => '/product/list.php?type=1',
		'feature' => 'takepos.catalog.manage_services'
	);
}
if ($canReadProducts && $isSaasFeatureEnabled('takepos.catalog.stock_overview')) {
	$productStudioLinks[] = array(
		'group' => 'manage',
		'label' => 'Stock Overview',
		'description' => 'Check stock movements and availability',
		'icon' => 'fa fa-warehouse',
		'url' => '/product/stock/list.php',
		'feature' => 'takepos.catalog.stock_overview'
	);
}
if ($canCreateCategories && $isSaasFeatureEnabled('takepos.catalog.add_category')) {
	$productStudioLinks[] = array(
		'group' => 'create',
		'label' => 'Add Category',
		'description' => 'Create a new product category',
		'icon' => 'fa fa-folder-plus',
		'url' => '/categories/card.php?action=create&type=0',
		'feature' => 'takepos.catalog.add_category'
	);
}
if ($canReadCategories && $isSaasFeatureEnabled('takepos.catalog.manage_categories')) {
	$productStudioLinks[] = array(
		'group' => 'manage',
		'label' => 'Category Manager',
		'description' => 'Browse and organize product categories',
		'icon' => 'fa fa-sitemap',
		'url' => '/categories/index.php?type=0',
...
TakeposSaasBridge::enforceFrontend($db, isset($user) ? $user : null, 'takepos.frontend', $term);
...
function setProductStudioState(openState) {
	var panel = document.getElementById("takeposProductStudioPanel");
	var backdrop = document.getElementById("takeposProductStudioBackdrop");
	if (!panel || !backdrop || !productStudioLinks.length) {
		productStudioOpen = false;
		return;
	}

	productStudioOpen = !!openState;
	if (productStudioOpen) {
		panel.classList.add("open");
		backdrop.classList.add("open");
		document.body.classList.add("takepos-product-studio-active");
	} else {
		panel.classList.remove("open");
		backdrop.classList.remove("open");
		document.body.classList.remove("takepos-product-studio-active");
	}
}

function toggleProductStudio(forceOpen) {
	if (!productStudioLinks.length) {
		return;
	}
	if (typeof forceOpen === "boolean") {
		setProductStudioState(forceOpen);
		return;
	}
	setProductStudioState(!productStudioOpen);
}

function closeProductStudio() {
	setProductStudioState(false);
}

function openProductStudioLink(index) {
	...
	$.getJSON("ajax/ajax.php", {
		...
		if (resp && (resp.allowed === 1 || resp.allowed === "1" || resp.allowed === true)) {
			openWorkspace();
		} else {
			alert("This action is disabled for your tenant plan.");
		}
	}).fail(function() {
...
$( document ).ready(function() {
	...
	$(document).on("keydown", function(event) {
		if (event.key === "Escape" && productStudioOpen) {
			closeProductStudio();
		}
	});
...
try { takeposCustomerDisplayChannel = new BroadcastChannel('takepos_customer_display'); } catch (e) { takeposCustomerDisplayChannel = null; }

function OpenCustomerDisplay() {
    var terminal = (typeof place !== 'undefined' ? place : '');
    var url = 'customer_display.php' + (terminal ? ('?terminal=' + encodeURIComponent(terminal)) : '');
    window.open(url, 'takepos_customer_display_window');
}

function parseTakeposMoney(textValue) {
    if (!textValue) return textValue;
    return String(textValue).replace(/\s+/g, ' ').trim();
}

function buildTakeposCustomerDisplayState() {
    var state = {
        invoiceId: ($('#invoiceid').val() || ''),
        invoiceRef: '',
        totalHt: '',
        ...
        message: 'Please review your order before payment.',
        footer: 'Thank you for shopping with us.',
        items: []
    };

    var spans = $('#poslines span[id="linecolht-span-total"]');
    if (spans.length > 0) {
        state.totalHt = parseTakeposMoney($(spans[0]).text());
    }
    if (spans.length > 1) {
        state.totalTtc = parseTakeposMoney($(spans[1]).text());
    } else {
        state.totalTtc = state.totalHt;
    }
    state.subtotal = state.totalHt;

    $('#poslines a[href*="compta/facture/card.php"], #poslines a[href*="facture/card.php"]').first().each(function(){
        state.invoiceRef = $(this).text().trim();
    });
    if (!state.invoiceRef && state.invoiceId) state.invoiceRef = '#' + state.invoiceId;

    $('#poslines tr.posinvoiceline').each(function(){
        var $row = $(this);
        ...
        var qty = '';
        var total = '';
        if (cells.length >= 2) qty = $(cells[cells.length-2]).text().replace(/\s+/g,' ').trim();
        if (cells.length >= 1) total = $(cells[cells.length-1]).text().replace(/\s+/g,' ').trim();
        if (label.toLowerCase().indexOf('totalttc') >= 0 || label.toLowerCase().indexOf('totalht') >= 0) return;
        state.items.push({ label: label, qty: qty || '1', total: total || '' });
    });

    if (!state.items.length) {
        state.message = 'Ready for the next customer.';
        state.footer = 'Open a sale on the cashier screen to start.';
    }
    return state;
}

function publishTakeposCustomerDisplay() {
    try {
        var state = buildTakeposCustomerDisplayState();
        ...
        console.log('Customer display publish failed', e);
    }
}

$(document).ready(function(){
    setTimeout(publishTakeposCustomerDisplay, 1200);
    setInterval(publishTakeposCustomerDisplay, 1500);
    if (window.MutationObserver && document.getElementById('poslines')) {
        var obs = new MutationObserver(function(){ publishTakeposCustomerDisplay(); });
        obs.observe(document.getElementById('poslines'), {childList:true, subtree:true, characterData:true});
    }
});
</script>
		...
		<div id="topnav" class="topnav">
			...
			<div id="topnav-right" class="topnav-right">
				...
				if ($reshook == 0) {  //Search method
					...
					<a onclick="toggleProductStudio(true);" class="nohover" title="Product Studio"><span class="fa fa-box-open"></span></a>
					<?php } ?>
...
<?php if ($productStudioEnabled) { ?>
<div id="takeposProductStudioBackdrop" class="takepos-product-studio-backdrop" onclick="closeProductStudio();"></div>
<aside id="takeposProductStudioPanel" class="takepos-product-studio-panel" aria-hidden="true">
	<div class="takepos-product-studio-head">
		<div>
			<h3>Product Studio</h3>
			<p>Create and manage products without leaving TakePOS.</p>
		</div>
		<button type="button" class="takepos-product-studio-close" onclick="closeProductStudio();" aria-label="Close Product Studio">
			<span class="fa fa-times"></span>
		</button>
	</div>
	<div class="takepos-product-studio-links">
	<?php foreach ($productStudioLinks as $idx => $item) { ?>
		<button type="button" class="takepos-product-studio-link takepos-product-studio-link-<?php echo dol_escape_htmltag($item['group']); ?>" onclick="openProductStudioLink(<?php echo (int) $idx; ?>);">
			<span class="takepos-product-studio-icon <?php echo dol_escape_htmltag($item['icon']); ?>"></span>
			<span class="takepos-product-studio-copy">
				<span class="takepos-product-studio-label"><?php echo dol_escape_htmltag($item['label']); ?></span>
				<span class="takepos-product-studio-desc"><?php echo dol_escape_htmltag($item['description']); ?></span>
			</span>
		</button>
	<?php } ?>
	</div>
	<div class="takepos-product-studio-footer">
		<button type="button" class="takepos-product-studio-refresh" onclick="refreshTakeposCatalog();">
			<span class="fa fa-sync-alt"></span> Refresh POS Catalog
		</button>
	</div>
</aside>
<button type="button" id="takeposProductStudioToggle" class="takepos-product-studio-toggle" onclick="toggleProductStudio();" title="Product Studio">
	<span class="fa fa-box-open"></span>
	<span class="takepos-product-studio-toggle-text">Product Studio</span>
</button>
<?php } ?>
...
<!-- Customer display launcher -->
<div id="takepos-customer-screen-launcher" style="position:fixed; right:18px; bottom:18px; z-index:99999;">
  <button type="button" onclick="OpenCustomerDisplay();" style="background:#2563eb;color:#fff;border:none;border-radius:999px;padding:12px 18px;font-size:14px;font-weight:bold;box-shadow:0 10px 24px rgba(0,0,0,.25);cursor:pointer;">🖥️ Customer Screen</button>
</div>
