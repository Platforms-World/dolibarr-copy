...
if ($action == 'checkfeature' && $user->hasRight('takepos', 'run')) {
	...
	$featureCode = GETPOST('feature_code', 'alphanohtml');
	$allowed = true;

	try {
		if ($featureCode !== '') {
			$allowed = TakeposSaasBridge::isFeatureEnabled($db, $featureCode);
		}
	} catch (Exception $e) {
		$allowed = false;
	}

	echo json_encode(array('allowed' => ($allowed ? 1 : 0), 'feature_code' => $featureCode));
	exit;
}
