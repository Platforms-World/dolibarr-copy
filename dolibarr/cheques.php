...
TakeposAccess::requireFrontendAccess(
    ...
    array('page' => 'cheques.php')
);
if (!TakeposChequeService::canRead($db, $user)) {
    TakeposAccess::denyAccess($db, $user, $langs->trans('TakeposChequeReadPermissionRequired'), array('page' => 'cheques.php'));
}
TakeposChequeService::ensureSchema($db);

$entity = !empty($user->entity) ? (int) $user->entity : 1;
$pageUrl = DOL_URL_ROOT . '/takepos/cheques.php';
...
</style></head><body class="<?php echo (!empty($langs->defaultlang) && strpos($langs->defaultlang, 'ar_') === 0 ? 'rtl' : ''); ?>">
...
<body class="takepos-cheques-body<?php echo (!empty($langs->defaultlang) && strpos($langs->defaultlang, 'ar_') === 0 ? ' rtl' : ''); ?>">
<div class="takepos-cheques-page">
    ...
    <div class="takepos-cheques-grid">
        <section class="takepos-panel">
            <h3><?php echo dol_escape_htmltag($current ? $langs->trans('TakeposChequeEditRecord') . ': ' . $current->ref : $langs->trans('TakeposChequeCreateRecord')); ?></h3>
            <form method="post" action="<?php echo dol_escape_htmltag($pageUrl . ($chequeId > 0 ? '?id=' . ((int) $chequeId) : '')); ?>">
                ...
                <input type="hidden" name="action" value="<?php echo dol_escape_htmltag($current ? 'update' : 'create'); ?>">
        ...
        <section class="takepos-panel">
            <h3><?php echo dol_escape_htmltag($langs->trans('TakeposChequeListTitle')); ?></h3>
