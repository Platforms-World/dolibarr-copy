...
class TakeposApiService
{
    ...
    /**
     * Persist the "current terminal" (and optionally the current shift) against
     * an API token. The stateless API has no PHP session, so the bearer token
     * itself remembers which terminal/shift the client selected via set_terminal.php.
     *
     * @param DoliDB   $db
     * @param int      $entity
     * @param int      $tokenId
     * @param int      $terminalId  Terminal rowid (0 to clear).
     * @param int|null $shiftId     Shift rowid, 0/null to clear.
     * @return bool
     */
    public static function bindTokenTerminal($db, $entity, $tokenId, $terminalId, $shiftId = null)
    {
        self::ensureSchema($db);

        $terminalSql = ((int) $terminalId > 0 ? (int) $terminalId : 'NULL');
        $shiftSql = ((int) $shiftId > 0 ? (int) $shiftId : 'NULL');

        $sql = "UPDATE " . self::tableApiToken()
            . " SET fk_terminal = " . $terminalSql
            . ", fk_shift = " . $shiftSql
            . " WHERE entity = " . ((int) $entity)
            . " AND rowid = " . ((int) $tokenId);
