...
// ── Kafo POS: redirect POS-only users to TakePOS ────────────────────────────
// Set by trigger InterfaceTakeposUserProvision on USER_LOGIN event.
// Admin users are never redirected. Does not touch existing permission system.
if (!empty($user->id) && empty($user->admin) && !empty($_SESSION['kafo_pos_redirect'])) {
    $kafo_redirect = $_SESSION['kafo_pos_redirect'];
    unset($_SESSION['kafo_pos_redirect']);
    header("Location: " . $kafo_redirect);
    exit;
}
// ── End Kafo POS redirect ─────────────────────────────────────────────────────
